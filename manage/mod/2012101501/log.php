...
//--------------------------------------------------------------------------------------------仙石记录
function LogStone($type) {
	global $db,$pdb,$cid,$sid,$server,$adminWebType,$page;

	$query = $pdb->query("select * from attribute_stone_change_type order by id asc ");
	...
	if ($username) 
	{
		...
		if ($usertype == 1) {
			$player = $pdb->fetch_first("select id from player where username = '$username'");

			//$player = api_admin::find_player_by_username($username);
			$player_id = $player['id'];
		}elseif ($usertype == 2){
			$player = $pdb->fetch_first("select id from player where nickname = '$username'"); 
		...
		select 
			count(*) 
		from 
			player_item_attribute_stone_log A 	
		...
		if($num)
		{
			...
			from 
				player_item_attribute_stone_log A
				left join attribute_stone_change_type B on A.type = B.id
				left join player C on A.player_id = C.id
			...
			$list_array_pages = multi($num,$pageNum,$page,$adminWebType.".php?in=player&action=Logs&type=stone&username=$username&usertype=$usertype&tidArr=$tidArr&stime=$stime&etime=$etime&cid=$cid&sid=$sid");	
		}
	}
	
	$pdb->close();
	include_once template('player_log_attribute_stone');
}
...
function LogSoul($type) {
	...
	if ($username) 
	{
		...
		if(!$f_db)
		{
			$s_f_db = 'player_soul_change_record2';
			$f_db = 0;
		}else{
			$s_f_db  = 'player_soul_change_record';
			$f_db = 1;
		}
	...
	if ($username || $fatename) 
	{
		...
		if($num)
		{
			...
			{	
				...
				if($rs['actived_fate_id1']) $fidArr[] = $rs['actived_fate_id1'];
				if($rs['actived_fate_id2']) $fidArr[] = $rs['actived_fate_id2'];
				$list_array[] =  $rs;
			}
