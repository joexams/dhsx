...
function  CallTemplates() 
{
	...
	{
		$server = $db->fetch_array($query);
		setcookie('game_mysql_admin', authcode($server['company_name'].'-'.$server['server_name']."@#$%".$server['db_server']."@#$%".$server['db_root']."@#$%".$server['db_pwd']."@#$%".$server['db_name'], 'ENCODE'),0,$cookiepath,$cookiedomain);
		//-----------------------------------------------------------------------------------------------
		
		if (setcookie('game_auth_admin', authcode($adminWebName."@#$%".$adminWebServersPower, 'ENCODE'),0,$cookiepath,$cookiedomain)){
