...
function  Player()
{
	...
	if ($type == 'test_1_2') 
	{
		...
		if($db->num_rows($query))
		{
			...
			{	
				$pay_array .=  $mrs['username'].',';
			}
			$pay_list = substr($pay_array,0,strlen($pay_array)-1);
			$pay_list = "'".str_replace(",", "','",$pay_list)."'";
			
			//echo $more_pay_list;
			$uquery = $db->query("
			select 
				distinct(A.username) AS username
			from 
				pay_data A
			where 
				A.cid = '$cid'
				and A.sid <> '$sid'
				and A.username in ($pay_list)
			group by 
				A.username
			");
			if($db->num_rows($uquery))
			{
				while($urs = $db->fetch_array($uquery))
				{	
					$more_array .=  $urs['username'].',';
				}
				
				$more_list = substr($more_array,0,strlen($more_array)-1);
				$more_list = "'".str_replace(",", "','",$more_list)."'";
				$set_type = "and A.username in ($more_list)";
				
			}else{
				$set_type = "and A.username = 'SDG35UY307@!356209!357=45'";
			}
	...
	if ($order) 
	{
		$select_com = ",sum(if(M.value < 0,M.value,0)) as consume_ingot";
	...
	if($num)
	{
		...
		$list_pages=multi($num,$pageNum,$page,$adminWebType.".php?in=player&username=$username_url&player_id=$player_id&ip=$ip&order=$order&type=$type&mt=$mt&source=$source&day=$day&level=$level&vip=$vip&st=$st&level_s=$level_s&level_e=$level_e&cid=$cid&sid=$sid");	
	}
...
/*
	if($adminWebType == 's')
	{
		...
		{				
			while($grs = $pdb->fetch_array($query))
			{	
				...
				from 
					player_gift_detail 
				where 
					player_gift_id = '$grs[id]'
				order by 
					item_id asc
				");
				
				
				if($pdb->num_rows($gquery))
				{				
					while($gdrs = $pdb->fetch_array($gquery))
					{
						$grs['player_gift_detail'][] = $gdrs;
					}
				}
...
*/	
	$consume_ingot = $pdb->result($pdb->query("
		select 
			sum(if(value < 0,value,0)) as consume_ingot
		from 
...
function DataConsume($type) {
	...
	if ($stime && $etime) 
	{
		$set_time = "AND B.change_time >= '$stime_s' AND B.change_time <= '$etime_e'";
	}
	if($order)
	{
	...
	if ($day) {
		$set_left_day = "left join player_trace E on C.id = E.player_id";
		$set_day = "and DATE_SUB(NOW(),INTERVAL $hour HOUR) >= DATE_FORMAT(FROM_UNIXTIME(E.last_login_time), '%Y-%m-%d %H:%i:%s') AND E.last_login_time > 0";
	}
	if($p == 1)
	{
		$query = $pdb->query("
		...
		$set_p = " and D.total_ingot > 0 AND C.vip_level >= 1";
		$set_left = "left join player_charge_record D on C.id = D.player_id";
	}elseif($p == 3){
		$set_p = " and D.total_ingot > 0 AND C.vip_level < 6";
		$set_left = "left join player_charge_record D on C.id = D.player_id";
	}elseif($p == 4){
		$set_p = " and D.total_ingot > 0 AND C.vip_level >= 6";
		$set_left = "left join player_charge_record D on C.id = D.player_id";
	}elseif($p == 5){
		$set_p = " and D.total_ingot > 0 AND C.vip_level >= 3";
		$set_left = "left join player_charge_record D on C.id = D.player_id";
	}
	if(!$set_time && !$username && !$set_day)//如果不在查询才使用缓存
	{
		$filename = $sid."_sxd_data_consume_".$t."_".$p."_".$order.".php";//文件名
		$dir = UCTIME_ROOT."/data/";//目录
		$flie = $dir.$filename;//全地址
		$filetime  = @filemtime($flie);//文件创建时间
		@include_once($flie);
	...
	//--------------------------------------------------------------
	if($is_update || $set_time || $username || $day)
	{
		$rs = $pdb->fetch_first("
		select 		
...
/*	if ($_COOKIE['sxd_setgamedata_'.$cid.'_'.$sid]) {
	...
	$today_s = $day ? strtotime(date('Y-m-d 00:00:00',strtotime($day))) : strtotime(date('Y-m-d 00:00:00'));
	$today_e = $day ? strtotime(date('Y-m-d 23:59:59',strtotime($day))) : strtotime(date('Y-m-d 23:59:59'));
	$today = $day ? $day : date('Y-m-d');//今日
	//$yesterday = date('Y-m-d',strtotime($today)-86400);//昨天数据
	
	$yesterday_s = strtotime(date('Y-m-d 00:00:00',strtotime($today)-86400));//昨天数据
	$yesterday_e = strtotime(date('Y-m-d 23:59:59',strtotime($today)-86400));//昨天数据
	...
	{	
		...
		$out_count = $pdb->result_first("
			...
			WHERE 
				C.level < 10
				AND B.first_login_time >= '$yesterday_s'
				AND B.first_login_time <= '$yesterday_e'			
		");	
		...
		where 
			B.total_ingot > 0
			AND C.is_tester = 0
			AND A.change_time >= '$today_s'
			AND A.change_time <= '$today_e'			
			AND A.change_time >= '$open_date'		
		...
		WHERE 
			...
			AND status = 0	
			AND success = 1	
			AND `dtime_unix` >= '$today_s'
			AND `dtime_unix` <= '$today_e'		
