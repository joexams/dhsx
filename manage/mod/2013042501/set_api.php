...
function  SetGiftFirstPay($player_id,$cid,$sid,$username) {
	...
	$odb->query("INSERT INTO ho_sys_log_activity(cid,sid,playerid,playername,playernickname,`key`,content,dateline) VALUES($cid, $sid, '$player_id','$username','$nickname','SetGiftFirstDay', '$show', ".time().");");
	return $ret;
}
...
function  SetGiftDays3($player_id,$pay_ingot,$open_date) {
	global $db,$cid,$sid,$openid,$nickname,$odb;
	...
	if (date('Y-m-d') >= $sdate &&  date('Y-m-d') <= $edate)//如果在活动时间范围内
	{
		...
		if ($ingot > 0) {
			...
			$odb->query("INSERT INTO ho_sys_log_activity(cid,sid,playerid,playername,playernickname,`key`,content,dateline) VALUES($cid, $sid, '$player_id','$openid','$nickname','SetGiftDays3', '$show', ".time().");");
		}
	}
}
