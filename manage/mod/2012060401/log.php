...
function LogFate($type) {
	...
	if ($username) 
	{
		...
		if($num)
		{
			...
			$list_array_pages = multi($num,$pageNum,$page,$adminWebType.".php?in=player&action=Logs&type=fate&username=".urlencode($username)."&usertype=$usertype&vip=$vip&tidArr=$tidArr&fatename=".urlencode($fatename)."&stime=$stime&etime=$etime&cid=$cid&sid=$sid");	
