...
function callPlayerStone() {
	...
	if($num){			
		...
		select 
			A.*,
			B.name as item_name,
			C.name as pack_grid_name,
			D.lv
		from 
			player_item_attribute_stone A
			left join item B on A.item_id = B.id
			left join item_pack_grid C on A.grid_id = C.id
			left join attribute_stone D on A.item_id = D.item_id
		where 
			A.player_id = '$uid' 
		order by 
			grid_id asc,
		...
		$list_array_pages = ajaxPage($num, $pageNum, $page, "player_call.php?action=callPlayerStone&sid=$sid&uid=$uid&winid=$winid", '',10,$winid);		
	}
	$pdb->close();
	include_once template('player_stone');
...
function callAchievement() {
	global $pdb,$sid;
	$query = $pdb->query("select * from achievement order by id asc");
	echo "obj.options[obj.options.length] = new Option('".languagevar('CHOOSE')."','0');\n"; 
...
function callPlayerDataConsumeLevel() 
{
	...
	if ($username) 
	{
		...
		if (!empty($parr)) $set_is_tester = " and A.player_id not in ($parr)";	
	}
