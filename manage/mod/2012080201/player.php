...
function DataConsume($type) {
	...
	if ($vip_s && $vip_e) 
	{
		$set_vip = "AND C.vip_level >= '$vip_s' AND C.vip_level <= '$vip_e'";
	}
	if ($level_s && $level_e) 
	{
		$set_level = "AND D.level >= '$level_s' AND D.level <= '$level_e'";
		$set_left = "left join player_role D on C.id = D.player_id and C.main_role_id = D.id";
	}	
...
*/	if(!$username && !$set_day)//如果不在查询才使用缓存
	{
		$filename = $sid."_sxd_data_consume_".$t."_".$order."_".$stime.$etime."_".$vip_s.$vip_e."_".$level_s.$level_e.".php";//文件名
		$dir = UCTIME_ROOT."/data/";//目录
		$flie = $dir.$filename;//全地址
		$filetime  = @filemtime($flie);//文件创建时间
		@include_once($flie);
	...
	if($is_update || $set_time || $username || $day)
	{
		...
		select 		
			...
			SUM(abs(B.value)+abs(B.change_charge_value)) AS value_count
		from 
			ingot_change_type A
	...
	//--------------------------------------------------------------
		...
		select 
			...
			SUM(if(A.id > 0,abs(B.value)+abs(B.change_charge_value),0)) AS value_count,
			A.id,
			A.name as type_name
		from 
			ingot_change_type A
