							...
							foreach ($targs as $key => $value) {
								if (array_key_exists($value['arg'], $info) && $value['arg'] != 'key'){
									...
									if (is_array($info[$value['arg']])) {
										$loginfo[] = $value['tips'].' = '.array2string($info[$value['arg']]);
									}else {
										$arg_name = $value['arg']."_name";
										$info_name = $info[$arg_name] ? $info[$arg_name] : $info[$value['arg']];
										$loginfo[] = $value['tips'].' = '.$info_name;
									}
								}
							}
