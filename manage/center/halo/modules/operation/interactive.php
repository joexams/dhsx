...
class interactive extends admin {
	...
	public function itemapply() {
		$has_check_priv = $this->has_priv('operation', 'check_itemapply', 'interactive');
	...
	public function retrieve() {
		$has_check_priv = $this->has_priv('operation', 'check_retrieve', 'interactive');
		include template('operation', 'retrieve');
	}
	/**
	 * 被盗找回申请
	 */
	public function add_retrieve()
	{
		if (isset($_POST['doSubmit'])) {
			...
			if ($sid > 0 && $id > 0) {
				...
				$retinfo = $retrievedb->get_one("userid=".$info['userid']." AND player_id=$id AND sid=$sid", '*', 'dateline DESC');
				$distime = abs(time() - intval($retinfo['dateline']));
				if ($distime < 60) {
			...
			include template('ajax', 'retrieve_add');
		}
	}
	...
	public function ajax_retrieve_list() {
		...
		if (!empty($playername)) {
			$wherestr .= !empty($wherestr) ? " AND (playername='$playername' OR nickname='$playername')" : "playername='$playername' OR nickname='$playername'";
		} 
	...
	public function delete_retrieve() {
		...
		if ($id > 0) {
			$retrievedb = common::load_model('retrieve_model');
			$rtn = $retrievedb->delete(array('id' => $id, 'status' => 1));
			if ($rtn) {
				output_json(0, Lang('success'));
			}
		}
	...
	public function add_itemapply() {
		if (!isset($_POST['doSubmit'])) {
			include template('ajax', 'itemapply_add');
			exit;
		}
	...
	public function check_itemapply() {
		...
		$has_check_priv = $this->has_priv('operation', 'check_itemapply', 'interactive');
		if ($has_check_priv === false)	output_json(1, Lang('no_permission'));
		...
		if ($checktype == 2) {
			...
			foreach ($list as $key => $value) {
				...
				if ($value['key'] == 'give_item') {
					$callback['result'] = $callback['success_number'] > 0 ? 1 : 0;
				}
				output_json($callback['result'], Lang('error'));
				if ($callback['result'] == 1){
					...
					$content['playerid'] = $value['player_id'];
					if ($value['key'] == 'give_item') {
						$content['content'] .= PHP_EOL.'实际获得数量：'.$callback['success_number'].'；未获得的数量：'.$callback['failure_number'].'。';
					}
		...
		output_json(3, Lang('error'));
	}

	/**
	 * 回复物品申请 
	 */
	public function reply_itemapply() 
	{
		$has_check_priv = $this->has_priv('operation', 'check_itemapply', 'interactive');
		if ($has_check_priv === false)	output_json(1, Lang('no_permission'));

		$aid = isset($_POST['aid']) ? intval($_POST['aid']) : 0;
		$reply_content = isset($_POST['replycontent']) ? trim($_POST['replycontent']) : '';
		if ($aid <= 0 || empty($reply_content))	output_json(1, Lang('reply').Lang('error'));

		$itemapplydb = common::load_model('itemapply_model');
		$rtn = $itemapplydb->update(array('reply_content'=>$reply_content, 'reply_time' => time(), 'reply_userid' => $_SESSION['userid'], 'reply_username' => param::get_cookie('username')), array('aid' => $aid));
		if ($rtn) {
			output_json(0, Lang('reply').Lang('success'));
		}
		output_json(2,  Lang('reply').Lang('error'));
	}
	/**
	 * 撤销
	 */
	public function del_itemapply()
	{
		$aid = isset($_GET['aid']) && intval($_GET['aid']) ? intval($_GET['aid']) : 0;
		if ($aid <= 0)	output_json(1, Lang('error'));
		
		$where = array('aid' => $aid, 'status' => 1);
		$roleid = $_SESSION['roleid'];
		if ($roleid > 3) {
			$userid = $_SESSION['userid'];
			$where['userid'] = $userid;
	...
	public function ajax_itemapply_list() {
		...
		if (!empty($keyword)) {
			$wherestr .= !empty($wherestr) ? " AND content LIKE '%$keyword%'" : " content LIKE '%$keyword%'";
		}
		if (!empty($playername)) {
			$wherestr .= !empty($wherestr) ? " AND player_name = '$playername'" : " player_name = '$playername'";
		}
		
		$wherestr = !empty($wherestr) ? str_ireplace('where', '', $wherestr) : '';
		$list = $itemapplydb->get_list_page($wherestr, '*', 'aid DESC', $page);
	...
	public function tester_refresh(){
		...
		if ($sid){
			...
			if ($dbflag){
				...
				$list = $this->getdb->select('is_tester in (1,2)', 'id,username, nickname');
				if ($list){
					$info['testers'] = serialize($list);
					$info['cid']	 = $cid;
					$info['sid']     = $sid;
					$this->getdb->table_name = 'player';
					$sql = "select id,username, nickname,@i:=@i+1 as `key`,is_tester from player,(SELECT @i:=0) AS rank where is_tester in (1,2)";
	...
	public function readbackup() {
		if ($_GET['dogetSubmit']) {
			...
			$wherestr = '';
			$finddb = common::load_model('find_backup_model');
			$list = $finddb->get_list_page($wherestr, '*', '', $page, 30);

			if ($recordnum <= 0) {
				$recordnum = $finddb->count($wherestr, 'id');
			}
			$data['count']  = $recordnum;
			$data['list']   = $list;
			...
			$has_apply_priv = $this->has_priv('operation', 'readbackup_apply', 'interactive');
			include template('operation', 'read_backup');
		}
	}
	/**
	 * 回档申请
	 */
	public function add_readbackup(){
		$has_apply_priv = $this->has_priv('operation', 'readbackup_apply', 'interactive');
		if ($has_apply_priv === false)	output_json(1, Lang('no_permission'));
		include template('ajax', 'read_backup_add');
	}
	/**
	 * 回档操作
	 */
	public function readbackup_apply() {
		$has_apply_priv = $this->has_priv('operation', 'readbackup_apply', 'interactive');
		if ($has_apply_priv === false)	output_json(1, Lang('no_permission'));
		if ($_POST['doSubmit']) {
			$func = 'read_backup';
			...
			$player_type = intval($_POST['player_type']);
			$playername = trim($_POST['player']);
			$backid = trim($_POST['backid']);
			$backtime = intval($_POST['backtime']);
			$applycontent = trim($_POST['applycontent']);
			...
			if (!empty($server['api_server']) && !empty($server['api_port']) && !empty($server['api_pwd']) && !empty($server['server_ver'])){
				...
				if ($api_admin !== false && method_exists($api_admin, $func)){
					...
					//通过昵称找玩家ID
					if ($player_type == 2){
						$player  = $api_admin::find_player_by_nickname($playername);
						$player1 = $api_admin::get_username_by_nickname($playername);
						$content['playername']     = $player1['username'][1];
						$content['playernickname'] = $playername;
					}else {
					//通过玩家名称找玩家ID
					...
					if ($player['result'] == 1){
						...
						$args = array($playerid, $backid);
						$callback = call_user_func_array(array($api_admin, $func), $args);

						if ($callback['result'] == 1){
							$content['content']  = Lang('log_op_reason').$applycontent;
							$content['key']      = $func;
							$content['sid']      = $sid;
			...
			$sid = intval($_GET['sid']);
			$nickname = $_GET['playername'];
			$username = $_GET['playername'];
			if (intval($_GET['player_type']) == 2){
				$wherestr = "nickname='$nickname'";
			}else {
				$wherestr = "username='$username'";
			}
			$flag = $this->set_db($sid);
			$this->getdb->table_name = 'player';
			$player = $this->getdb->get_one($wherestr, 'id, username');
			if (!$player) 	output_json(1, '');
			$player_id = $player['id'];
			$this->getdb->table_name = 'player_back';
			$backlist = $this->getdb->select(array('player_id' => $player_id), 'id, back_time');
			$data['list']   = $backlist;
