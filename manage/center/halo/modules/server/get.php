...
class get extends admin {
	 ...
	 * 
	 */ 
	public function dragonball_list(){
		$sid = isset($_GET['sid']) ? intval($_GET['sid']) : 0;
		$dbflag = $this->set_db($sid);

		if ($dbflag){
			$sql = 'SELECT a.id as dragonballid, a.name as dragonballname, b.name as quality FROM dragonball a 
					LEFT JOIN dragonball_quality b ON a.quality_id=b.id 
					WHERE a.quality_id > 1 
					ORDER BY a.quality_id ASC;';
			$data['list'] = $this->getdb->get_list($sql);
		...
		if ($dbflag){
			...
			switch ($order) {
				...
				case 'vip':
					...
					break;
				case 'ingot':
					$str_order = 'b.ingot+b.charge_ingot DESC';
					break;
				case 'coins':
			...
			if ($recordnum <= 0 && $list) {
				if (isset($_GET['ip']) && !empty($_GET['ip'])) {
					...
					$countsql = "SELECT COUNT(a.id) as num FROM player a LEFT JOIN player_data b ON a.id=b.player_id LEFT JOIN player_role c ON a.id=c.player_id AND a.main_role_id=c.id LEFT JOIN player_mission_record J on a.id = J.player_id and J.is_finished = 1 LEFT JOIN mission I on I.id = J.mission_id $set_lock_m $set_wyh $wherestr;";
					$count = $this->getdb->get_count($countsql);
				}else {
...
HTML;
				...
				if ($typeflag == 0){
					...
					switch ($key) {
						...
						case 'defeat_world_boss':
							...
							$data['type']['listinfo'] = $this->getdb->get_list($sql);
							break;
						case 'blood_pet_chip':
							$this->getdb->table_name = 'item';
						...
						case 'blood_pet':
							...
							$data['type']['item'] = $this->getdb->select('type_id=45000', 'id,name');
							break;
						case 'reincarnation_book':
							$this->getdb->table_name = 'reincarnation_stunt_book';
							$data['type']['listinfo'] = $this->getdb->select('', 'id,name');
							break;
						case in_array($key,array('dragonball','mission','five_elements_material')):
							$this->getdb->table_name = $key;
							$data['type']['listinfo'] = $this->getdb->select('', 'id,name');
							break;
					}
	...
	public function server_player_stat(){
		...
		if (file_exists(ROOT_PATH.'../online_data.php')){
			...
			$data['online'] = isset($online_data[$name]) && $combined_to==0 ? $online_data[$name] : 0;
		}
		//收入
	...
	private function set_player_search($get){
		...
		$is_online    = isset($get['is_online']) ? intval($get['is_online']) : 0;
		...
		if ($is_online > 0){
			...
			$wherestr .= !empty($wherestr) ? " AND is_online='$is_online'" : " WHERE is_online='$is_online'";
		}
	...
	private function player_key_record($tbl_key, $get){
		...
		if (!empty($typeid)){
			$type = in_array($tbl_key, array('fame', 'fate', 'power', 'take_bible', 'skill', 'coin_tree_count', 'long_yu_ling', 'xian_ling', 'xianling_tree', 'crystal', 'marry_favor', 'pearl', 'feats','blood_pet_chip','blood_pet','ling_yun','neidan','ba_xian_ling','marry_gold','five_elements_material','reincarnation_book')) ? 'op_type' : 'type';
		...
		if (in_array($tbl_key, array('fame','power','blood_pet_chip','feats','ling_yun','neidan','blood_pet','take_bible','skill', 'coin_tree_count','pearl','ba_xian_ling','marry_gold','five_elements_material','reincarnation_book'))){
			$time_column = 'op_time';
		}
		...
		if (!empty($name)) {
			...
			switch ($tbl_key) {
				...
				case 'blood_pet_chip':
					...
					break;
				case 'five_elements_material':
					$this->getdb->table_name = 'five_elements_material';
					$nameid = 'material_id';
					$nameitem = $this->getdb->select($namewhere, 'id, name');
					break;
				case 'reincarnation_book':
		...
		if (in_array($tbl_key, array('fame', 'fate', 'power', 'take_bible', 'skill', 'farmland', 'flower_count', 'coin_tree_count', 'elixir', 'item_attribute_stone', 'faction_contribution', 'spirit','xian_ling', 'xianling_tree', 'long_yu_ling', 'crystal', 'marry_favor', 'pearl', 'feats', 'dragonball','blood_pet_chip','blood_pet','ling_yun','neidan','ba_xian_ling','marry_gold','five_elements_material','reincarnation_book'))){
