...
class server extends admin {
	...
	public function public_server_list(){
		...
		if ($all > 0){
			...
			if ($cid > 0){
				...
				$wherestr .= !empty($wherestr) ? " and open=1 and combined_to=0 and open_date<now() and cid=1": " open=1 and combined_to=0 and open_date<now() and cid=1";
			}
			$data['scount'] = array(
			'wait' => 0,
			'notsetting' => 0,
			...
			if (isset($_GET['dogetSubmit'])){
				...
				if (isset($_GET['name']) && !empty($_GET['name'])){
					$name = 'qq_s'.trim(safe_replace($_GET['name']));
					$wherestr .= !empty($wherestr) ? " AND name = '$name'" : " name = '$name'";
				}
				...
				if (isset($_GET['url']) && !empty($_GET['url'])){
					$url = trim(safe_replace($_GET['url']));
					$wherestr .= !empty($wherestr) ? " AND server like '%".$url."%'" : " server like '%".$url."%'";
				}
				if (isset($_GET['apis']) && !empty($_GET['apis'])){
					$apis = trim(safe_replace($_GET['apis']));
					$wherestr .= !empty($wherestr) ? " AND api_server like '%".$apis."%'" : " api_server like '%".$apis."%'";
				}
				if (isset($_GET['dbs']) && !empty($_GET['dbs'])){
					$dbs = trim(safe_replace($_GET['dbs']));
					$wherestr .= !empty($wherestr) ? " AND db_server like '%".$dbs."%'": " db_server like '%".$dbs."%'";
				}
				...
				if (isset($_GET['combined_to']) && !empty($_GET['combined_to'])){
					$_GET['combined_to'] == 2 ? $equal = "=" : $equal = ">";
					$wherestr .= !empty($wherestr) ? " AND combined_to $equal 0": " combined_to $equal 0";
				}
				if (isset($_GET['opendate']) && !empty($_GET['opendate']) && $_GET['opendate'] != 0){
					$opendate = trim(safe_replace($_GET['opendate']));
					$wherestr .= !empty($wherestr) ? " AND  left(open_date,10)='".$opendate."'": "  left(open_date,10)='".$opendate."'";
				}
			...
			$column = 'cid,sid,name,o_name,api_server,api_port,server,db_server,db_name,open_date,unix_timestamp(open_date) as opendate,server_ver,open,test,private,combined_to';
			if ($type == 'list'){
			...
			$list = $this->serverdb->get_list_page($wherestr, $column, 'opendate DESC', $page, $this->pagesize);
			if ($recordnum <= 0){
		...
		foreach ($list as $key => $value) {
			...
			if ($all<=0){
				if ($value['combined_to']) {
					$list[$key]['logserver'] = $value['combined_to'];
				}else {
					$list[$key]['logserver'] = '';
				}
			}
	...
	public function ajax_setting_list(){
		...
		if ($all > 0){
			...
			$column = 'cid,sid,name,o_name,api_server,api_port,server,db_server,db_name,open_date,unix_timestamp(open_date) as opendate,server_ver,open,test,private,combined_to';
			if ($type == 'list'){
				$column = 'cid,sid,name,o_name,api_server,server,open_date,unix_timestamp(open_date) as opendate,combined_to';
			}
			$wherestr = str_ireplace('where', '', $wherestr);
	...
	public function ajax_setting_info(){
		...
		if ($sid > 0){
			...
			$addresslist = $this->serverdb->select('','*','','name');
	...
	public function ajax_address_list(){
		...
		$api_address = isset($_GET['api_address']) ? trim($_GET['api_address']) : '';
		$this->serverdb->table_name = 'servers_address';
		if (isset($_GET['all'])){
			$wherestr = $type > 0 ? 'type='.$type : '';
			if ($api_address){
				$wherestr .= !empty($wherestr) ? " AND CONCAT(name3,name) like '%$api_address%'": "CONCAT(name3,name) like '%$api_address%'";
			}
			$list = $this->serverdb->select($wherestr, '*', '', 'id DESC');
		}else {
			$page        = isset($_GET['top']) && intval($_GET['top']) > 0 ? intval($_GET['top']) : 1;
			$recordnum   = isset($_GET['recordnum']) ? intval($_GET['recordnum']) : 0;
			$type == 1 ? $this->pagesize    = 500 : $this->pagesize    = 50;
			if ($api_address){
				$where = " AND concat(name3,name) like '%$api_address%'";
			}
			$list = $this->serverdb->get_list_page("type='$type'".$where, '*', 'id DESC', $page, $this->pagesize);
			...
			if (count($list) > 0){
				...
				switch ($type) {
					case 0:
						...
						$wherestr1 = ' api_server IN ('.$str_names.') and combined_to=0';
						$wherestr2 = ' api_server IN ('.$str_names.') and combined_to!=0';
						$column   = 'api_server';
						break;
					case 1:
				...
				$wherestr1 ? $typelist1 = $this->serverdb->select($wherestr1, "count(sid) as num1,$column", '', '', $column) : '';
				$wherestr2 ? $typelist2 = $this->serverdb->select($wherestr2, "count(sid) as num2,$column", '', '', $column) : '';
				foreach ($list as $key => $value) {
					...
					if ($typelist1){
						foreach ($typelist1 as $tval1) {
							if ($tval1[$column] == $value['name']){
								$list[$key]['count1'] = $tval1['num1'];
								break;
							}
						}
					}
					if ($typelist2){
						foreach ($typelist2 as $tval2) {
							if ($tval2[$column] == $value['name']){
								$list[$key]['count2'] = $tval2['num2'];
								break;
							}
						}
					}
				...
				unset($str_name, $typelist, $typelist1, $typelist2);
			}
			if ($recordnum <= 0){
				$this->serverdb->table_name = 'servers_address';
				$recordnum = $this->serverdb->count('type='.$type.$where, 'id');
			}
		}
		foreach ($list as $k => $v){
			$count1[$k] = $v['count1'];
		}
		$type==0 ? array_multisort($count1,$list) : '';
		$data['count']  = $recordnum;
		$data['list']   = $list;
		$data['type']   = $type;
	...
	public function combine() {
		...
		if (isset($_POST['doSubmit'])) {
			...
			$combineid = isset($_GET['combineid']) && intval($_GET['combineid']) > 0 ? intval($_GET['combineid']) : 0;
			$info['cid'] = $cid;
	...
	public function combined_point() {
		...
		if ($combinedid) {
			...
			if (!empty($sids)) {
				...
				$serverlist = $this->serverdb->select("sid IN (".$sids.")", 'open_date,open_date_old', '', 'open_date ASC');
				foreach ($serverlist as $key => $value) {
					$odate[] = $value['open_date_old'] != '0000-00-00 00:00:00' ? $value['open_date_old'] : $value['open_date'];
				}
				$this->serverdb->update(array('open_date_old' => min($odate)), array('sid'=>$serverid));
				foreach ($sidarr as $skey => $svalue) {
					$combined_to='';
					$combined_to = $this->serverdb->select("combined_to=$svalue", "sid");
					if ($combined_to){
						$this->serverdb->update(array('is_use' => 0), array('sid'=>$svalue));
						foreach ($combined_to as $v){
							$this->serverdb->update(array('combined_to' => $serverid), array('sid'=>$v["sid"]));
						}
					}else{
						$this->serverdb->update(array('combined_to' => $serverid), array('sid'=>$svalue));
					}

				}
				$mergedb->update(array('status' => 1), array('id' => $combinedid));
				$content = Lang('success_combined_point')."(SID=".$serverid.")";
				parent::op_log($content);
	...
	public function ajax_combine_list() {
		$page        = isset($_GET['top']) && intval($_GET['top']) > 0 ? intval($_GET['top']) : 1;
		$recordnum   = isset($_GET['recordnum']) ? intval($_GET['recordnum']) : 0;
		$opendate    = isset($_GET['opendate']) ? intval($_GET['opendate']) : '';
		$daytype    = isset($_GET['daytype']) ? intval($_GET['daytype']) : '';
		if ($opendate){
			$where = "FROM_UNIXTIME(opendate,'%Y%m%d')='$opendate'";
		}
		if ($daytype){
			switch ($daytype) {
				case 1:
		...
		$this->mergedb = common::load_model('server_merge_model');
		$list = $this->mergedb->get_list_page($where, '*', 'id DESC', $page, $this->pagesize);
		if ($recordnum <= 0){
			$recordnum = $this->mergedb->count($where, 'id');
		}
	 ...
	 * 
	 */
	public function combine_date_list() {
		$pubdb = common::load_model('public_model');
		$pubdb->table_name = 'ho_pf_servers_merge';
		$date_list = $pubdb->get_list("select FROM_UNIXTIME(opendate,'%Y%m%d') as open_date,count(*) as num  from ho_pf_servers_merge group by open_date order by open_date desc");
		$data['list']  = $date_list;
		output_json(0, '', $data);
	}
	/**
	 ...
	 * 
	 */
	public function open_date_list() {
		$pubdb = common::load_model('public_model');
		$pubdb->table_name = 'servers';
		$date_list = $pubdb->get_list("select left(open_date,10) as open_date,count(*) as num from servers where open=1 AND open_date<'".date('Y-m-d H:i:s')."' group by open_date order by open_date desc");
		$data['list']  = $date_list;
		output_json(0, '', $data);
	}
	/**
	...
	public function test_db_connect(){
		...
		output_json($status, $msg, array('sid' => $sid ,'msg' => $msg));
	}
	/**
	 * 获取目录信息
	...
	public function sql_tool() {
		if (isset($_GET['doSubmit'])) {
			$sname = "qq_s".$_GET['sname'];
			$sql = stripslashes(trim($_GET['sql']));
			$limit = ReqNum('limit');
			$game_manage = $_GET['game_manage'];
			$exec_like_sql = $_GET['exec_like_sql'];
			//			$sname = 'trunk';
			$arr = explode(" ",$sql);
			if (strtolower($arr[0]) == 'select' || strtolower($arr[0]) == 'show'){
				$pubdb = common::load_model('public_model');
				if ($exec_like_sql != 1){
					strtolower($arr[0]) == 'select' ? $sql = $sql." limit ".$limit : $sql = $sql;
				}
				if ($game_manage == 1 && $_SESSION['roleid'] == 1){
					$list = $pubdb->get_list($sql);
				}else{
					$server = $this->serverdb->get_one(array('name'=>$sname));
					if ($server){
						$sid = $server['sid'];
						$combined_to = $server['combined_to'];
						$sid = $combined_to>0 ? $combined_to : $sid;
						$getdb = $pubdb->set_db($sid);
						$getdb !== false ? $list = $getdb->get_list($sql) : output_json(1, Lang('error'));
					}else{
						output_json(1, Lang('can_not_find_game_server'));
					}
				}
				if (isset($list)){
					...
					output_json(1, Lang('check_sql'));
				}
			}else{
				output_json(1, Lang('only_select_show_sql'));
			}
		}else{
			include template('develop', 'sql_tool');
		}
	}
		...
		if (isset($_POST['doSubmit'])) {
			...
			if (strtolower($arr[0]) == 'select' || strtolower($arr[0]) == 'show'){
				if ($cid > 0 && count($sid) > 0){
					...
					foreach ($sid as $key => $value){
						$server = $serverdb->get_one(array('sid' => $value), 'name');
						$servername = substr($server['name'],4);
						$str .= PHP_EOL."{".$servername.",[";
						$getdb = $serverdb->set_db($value);
						$getdb !== false ? $list = $getdb->sql_query($sql) : output_json(1, Lang('error'));
						$fields_num = mysql_num_fields($list);
					...
					output_json(1, Lang('please_select_server'));
				}
			}else{
				output_json(1, Lang('only_select_show_sql'));
			}
		}else{
			include template('develop', 'run_data');
		}
	}
}
