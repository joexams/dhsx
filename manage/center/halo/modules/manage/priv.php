...
class priv extends admin {
	...
	public function ajax_show(){
		...
		$roleid_menu = isset($_SESSION['roleid']) ? intval($_SESSION['roleid']) : 0;
		$userid_menu = isset($_SESSION['userid']) ? intval($_SESSION['userid']) : 0;
		...
		$user = $userdb->get_one(array('userid' => $userid_menu), 'isrolepriv');
        if ($user['isrolepriv'] == 1) {
			$this->privdb->set_model(1);
			$priv_menu = $this->privdb->select("userid='$userid_menu'", 'mid');
        }else {
        	if ($roleid_menu>1){
        		$this->privdb->set_model(0);
				$priv_menu = $this->privdb->select("roleid='$roleid_menu'", 'mid');
			}
        }
        $list = array();
        foreach ($priv_menu as $key_menu => $value_menu){
        	$list[] = $value_menu['mid'];
        }
        $wherestr = $priv_menu ? to_sqls(array_values($list), '', 'mid') : $wherestr;
