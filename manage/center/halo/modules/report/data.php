...
class data extends admin {
	...
	public function total(){
		...
		if ($cid > 0){
			...
			$sql = "SELECT sid,SUM(`pay_amount`) AS pay_amount,SUM(`new_player`) AS newer_count, SUM(pay_player_count) AS pay_player_count, SUM(`pay_num`) AS pay_num, SUM(`register_count`) AS register_count,SUM(`create_count`) AS create_count, SUM(login_count) AS login_count, SUM(`avg_online_count`) AS online_count, max(`max_online_count`) AS max_online_count, SUM(consume) AS consume FROM game_data $wherestr GROUP BY sid";
