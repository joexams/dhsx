...
class querytool extends admin {
	...
	public function regincome() {
		if (isset($_GET['doSubmit'])) {
			...
			foreach ($serverlist as $key => $server) {
				...
				$getdb = new getdb_model($dbconfig, $server['db_name']);
				...
				$paywhere1 = '';
				$paywhere1 = 'sid='.$server['sid'].''.(!empty($paywhere) ? ' AND '.$paywhere : '');
				$paylist = $this->pubdb->select($paywhere1, 'player_id, amount');
	...
	public function ingotstock() {
		if (isset($_GET['doSubmit'])) {
			...
			$this->pubdb->table_name = 'pay_data';
			$amount = $this->pubdb->get_one($wherestr, 'SUM(coins) AS amount');
