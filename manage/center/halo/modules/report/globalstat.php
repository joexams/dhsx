		...
		if (isset($_GET['dogetSubmit'])) {
			...
			if ($this->getdb !==  false){
				...
				$sql = "SELECT a.mission_id, d.name AS town, b.name AS mission, SUM(times) AS pktimes,SUM(failed_challenge) AS pkfailedtimes,COUNT(CASE WHEN is_finished=1 THEN 1 ELSE NULL END) AS finished , COUNT(CASE WHEN is_finished=0 THEN 1 ELSE NULL END) AS notfinished FROM player_mission_record a 
						...
						$wherestr GROUP BY b.lock ORDER BY b.lock DESC LIMIT $offset, $pagesize";

				$data['list'] = $this->getdb->get_list($sql);
				$recordnum = $this->getdb->get_list("select a.mission_id from player_mission_record a LEFT JOIN mission b ON a.mission_id=b.id $wherestr GROUP BY b.lock");
				...
				$data_list['count'] = $data['count'];
				output_json(0, '', $data_list);
			}
	...
	public function level(){
		if (isset($_GET['doSubmit'])) {
			...
			if ($cid > 0 && count($arrsid) > 0){
				...
				$sql = trim($sql, ',');
				$sql .= " FROM player a LEFT JOIN player_role b ON a.id=b.player_id AND a.main_role_id=b.id LEFT JOIN role c ON b.role_id=c.id WHERE is_tester=0 $wherestr GROUP BY gender";
