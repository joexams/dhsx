...
function blockManage(bid, pid, menuname){
	...
	bid = parseInt(bid);
	pid = parseInt(pid);
	var title = bid > 0 ? '<?php echo Lang('edit_block')?>' : '<?php echo Lang('add_block')?>';
	var queryData = 'bid='+bid;
	if (pid > 0) {
		title = '<?php echo Lang('add_menuname_block')?>';
		queryData += '&pid='+pid;
	}
...
<div class="column whitespace cf" id="table_column">
	<div class="title">
        <div class="more" id="tblMore">
            <div id="div_pop">
                <input class="btn_thin1" type="button" onclick="blockManage(0)" value="<?php echo Lang('add_block')?>">
            </div>
        </div>
	...
	<div class="column cf">
		...
		<thead>
			<th><?php echo Lang('block_name')?></th>
