...
<option value="${cid}" rel="#app=4&url=${encodeurl('<?php echo $data['url']['m']; ?>', '<?php echo $data['url']['v']; ?>', '<?php echo $data['url']['c']; ?>', '&cid=')}${cid}" {{if cid == <?php echo $cid;?>}} selected="selected" {{/if}}>${fn} - ${name}</option>
</script>
...
<div class="container" id="container">
	<div class="column cf" id="table_column">
		...
		<tr id="dataTheadTr">
			...
			<th><?php echo Lang('yesterday_avg_online'); ?></th>
		  ...
		  <?php if (!empty($list)) { ?>
		  	<?php foreach ($list as $key => $value) { ?>
		  		<tr>
		  		    ...
		  		    <td><?php echo $value['avg_online_count'] ? $value['avg_online_count'] : '--';?></td>
