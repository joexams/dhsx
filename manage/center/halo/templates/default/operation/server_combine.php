...
function getList(index){
	var query = "<?php echo INDEX; ?>?m=operation&c=server&v=ajax_combine_list&top="+index+"&recordnum="+recordNum;
	pageIndex = index;
		...
		for (var i=0; i<sids.length; i++) {
			for (var key in global_serverlist) {
				if (global_serverlist[key].sid == sids[i]) {
					names += global_serverlist[key].name + '-' + global_serverlist[key].o_name + '<br>';
					break;
				}
			}
		}
...
$(function(){
	...
	$('#post_submit').on('submit', function(e) {
		...
		if (cid > 0 && sid.length == 2){
			$.ajax({
				url: '<?php echo INDEX; ?>?m=operation&c=server&v=combine',
				type: 'post',
...
<tr>
	...
	<span class="graytitle"><?php echo Lang('register_seven_day_get_combined_compensation')?></span>
	</td>
	<td><a href="javascript:;" class="edit" data-id="${id}"><?php echo Lang('edit'); ?></a></td>
	<td>&nbsp;</td>
</tr>
</script>
...
<div id="bgwrap">
	...
	<div class="content" id="submit_area">
		<!-- Begin form elements -->
		<form name="post_submit" id="post_submit" action="<?php echo INDEX; ?>?m=operation&c=server&v=combine" method="post">
		...
		<table class="global" width="100%" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
					...
					<th style="width:10%"><?php echo Lang('combined_server'); ?></th>
					<th style="width:10%"><?php echo Lang('taget_server'); ?></th>
					<th style="width:20%"><?php echo Lang('remark'); ?></th>
