...
function getlogxian_lingList(index){
	var query = "<?php echo INDEX; ?>?m=server&c=get&v=player_record", tbl = $('#tbl').val()
	params = {sid: '<?php echo $data['sid']; ?>', id: '<?php echo $data['id']; ?>', key: 'xian_ling', typeflag: typeflag, top: index, recordnum: recordNum, tbl: tbl};
	pageIndex = index;
	$( "#list" ).fadeOut( "medium", function () {
		$.ajax({
			...
			url: query,
			data: params,
			success: showlogxian_lingList
		});
	});
	return false;
}
function getsearchlogxian_lingList(index){
	...
	$( "#list" ).fadeOut( "medium", function () {
		$.ajax({
			dataType: "json",
			url: query,
			data: $('#get_search_xian_ling_submit').serialize(),
			success: function(data){
				showlogxian_lingList(data, 1);
			}
		});
	});
	return false;
...
function showlogxian_lingList( data, type ) {
	if (data.status == 1){
		$('#list').html(data.msg);
	}else {
		if (data.type != undefined){
			typeflag = 1;
			typelist = data.type;
			$('#xian_lingtypetpl').tmpl( data.type.get ).appendTo('#xian_lingget');
			$('#xian_lingtypetpl').tmpl( data.type.cons ).appendTo('#xian_lingcons');
		}
		...
		if (type != undefined && type == 1){
			$( "#xian_lingpager" ).pager({ pagenumber: pageIndex, pagecount: pageCount, word: pageword, buttonClickCallback: getsearchlogxian_lingList });
		}else {
			$( "#xian_lingpager" ).pager({ pagenumber: pageIndex, pagecount: pageCount, word: pageword, buttonClickCallback: getlogxian_lingList });
		}
		$( "#list" ).empty();
...
$(document).ready(function(){
	if (typeof dialog != 'undefined'){
		dialog.title("<?php echo Lang('player').urldecode($_GET['title']).'->'; ?>"+'<?php echo Lang('xian_ling').Lang('log'); ?>');
	}
	getlogxian_lingList( pageIndex );
	
	$('#get_search_xian_ling_submit').on('submit', function(e){
		e.preventDefault();
		recordNum = 0;
		getsearchlogxian_lingList(1);
	});
});
</script>
<script type="text/template" id="xian_lingtypetpl">
<li><a href="javascript:;" data-id="${id}" class="xian_ling_type">${name}</a><span></span></li>
</script>
...
<div id="bgwrap">
	<form id="get_search_xian_ling_submit" action="<?php echo INDEX; ?>?m=server&c=get&v=player_record" method="get" name="form">
	<div class="onecolumn">
		<div class="header">
		...
		<ul class="nav_li">
			...
			<li>
				<p>
					...
					<input type="hidden" name="key" value="xian_ling">
	...
	<div class="pagination" id="xian_lingpager"></div>
