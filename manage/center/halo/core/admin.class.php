...
class admin {
	...
	final public static function admin_menu($parentid, $with_self = 0, $isrolepriv = 0, $get_all = 0, $display = 0) {
		...
		if($_SESSION['roleid'] == 1) {
			$result =$menudb->select(array('parentid'=>$parentid,'display'=>$display), 'mid,mname,parentid,m,v,c,parentpath,data,depth,islink,urllink',10,'listorder ASC');
			if ($get_all == 1){
				$result =$menudb->select(array('parentid'=>$parentid,'display'=>$display), 'mid,mname,parentid,m,v,c,parentpath,data,depth,islink,urllink','','listorder ASC');
			}
			if($with_self) {
