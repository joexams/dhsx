...
foreach ($serverlist as $key => $value){
	...
	if ($getdb !== false){
		...
		$sql = "select player_id,sum(value+change_charge_value) sum from player_ingot_change_record where change_time between $starttime and $endtime and (value<0 or change_charge_value<0 ) group by player_id order by sum";
		...
		foreach ($list as $pkey => $pvalue){
			...
			if ($player_id>0 && $sum>999){
				$number = floor($sum/1000);
				$item_list = array(array('item_id'=>2180,'number'=>$number),array('item_id'=>2181,'number'=>$number),array('item_id'=>2182,'number'=>$number));
				$message = '恭喜你获得消费活动礼包';
