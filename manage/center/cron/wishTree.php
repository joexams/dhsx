...
foreach ($serverlist as $key => $value){
	...
	if ($getdb !== false){
		...
		$sql = "select player_id from player_wish_tree where wish_time<1417276800 and red_bless_bag>9 order by red_bless_bag desc limit 1";
		$list = $getdb->get_list($sql);
		foreach ($list as $pkey => $pvalue){
			$item_list = array();
			$player_id = $pvalue['player_id'];
			if ($player_id>0){
				$item_list = array(array('item_id'=>2213,'number'=>30));
				$message = '恭喜你获得许愿树排行第一礼包';
