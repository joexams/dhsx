(function(c) {
	c.fn.pager = function(d) {
		...
		return this.each(function() {
			c(this).empty().append(b(parseInt(d.pagenumber), parseInt(d.pagecount), parseInt(d.showfirst), parseInt(d.showlast), d.buttonClickCallback, d.word, d.clickCallbackFunction));
	...
	function b(i, h, g, f, m, p, q) {
		...
		if (h > 1) {
			...
			if (d > 1) {
				c('<a href="javascript:;">1</a>').click(function(){m(this.firstChild.data,q);}).appendTo(e);
			...
			if (h > k) {
				c('<span>...</span>').appendTo(e);
				c('<a href="javascript:;">'+(h)+'</a>').click(function(){m(this.firstChild.data,q);}).appendTo(e);
			}
			if (i != h) {
				e.append(a("next", i, h, m, p, q));
				// if (isNaN(f)) {
				// 	e.append(a("last", i, h, m, p));
				// }
			}
			var t = c('<input type="text" name="pageNum" size=2>').keyup(function(ev){
				var ev = ev || event,
