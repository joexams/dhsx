...
Ha.page = {
	...
	getList: function(index, callback){
		Ha.page.pageIndex = index;
		Ha.mask.show(Ha.page.maskEid);
		$.ajax({
			dataType: Ha.page.dataType,
			url: Ha.page.url+"&top="+index+"&recordnum="+Ha.page.recordNum,
			data: Ha.page.queryData,
			success: function(data){
				if (typeof(callback) == 'function') {
					callback(data);
				}
				Ha.page.showList(data,callback);
			},
			complete: function(){
				Ha.mask.clear(Ha.page.maskEid);
			},
			error: function(XMLHttpRequest, textStatus, errorThrown){
				Ha.notify.show('系统出错了：'+textStatus, '', 'error');
			}
		});
	},
	showList: function(data,callback){
		if (data.status > 0){
			Ha.notify.show(data.msg, '', 'error');
		}else {
			Ha.page.recordNum = data.count;
			Ha.page.pageCount = Math.ceil( data.count/Ha.page.pageSize );
			$( "#pager" ).pager({ pagenumber: Ha.page.pageIndex, pagecount: Ha.page.pageCount, word: Ha.page.word, buttonClickCallback: Ha.page.getList,clickCallbackFunction: callback });
			if (data.count > 0){
				$( "#"+ Ha.page.listEid).empty().append($( "#"+Ha.page.listEid+"tpl" ).tmpl( data.list )).show();
			}else {
				$( "#"+ Ha.page.listEid).html('<tr><td colspan="'+Ha.page.colspan+'" style="text-align: left">'+Ha.page.emptyMsg+'</td></tr>');
			}
		}
	}
};
...
Ha.common = {
	...
	show: function(data) {
		var alertclassname = '',alerttype = '';
		switch (data.status){
			case 0: alertclassname = 'success'; alerttype = 'normal'; break;
			case 1: alertclassname = 'error'; alerttype = 'error'; break;
		}
		Ha.notify.show(data.msg, alerttype, alertclassname);
	},
...
Ha.Dialog = {
	show: function(content, title, width, id, extra){
		var opts = {
			title: title || '提示信息',
			content: content || '',
			id:  id || '',
		...
		if (typeof(extra) != 'undefined') {
			$.dialog({id: opts.id, width: opts.width, title: opts.title, content: opts.content, follow: extra, lock: true});
		}else {
			$.dialog({id: opts.id, width: opts.width, title: opts.title, content: opts.content, lock: true});
		}
	}
};
