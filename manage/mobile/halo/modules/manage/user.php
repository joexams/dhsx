...
class user extends admin {
	...
	public function ajax_list() {
		...
		$menuname = isset($_GET['menuname']) && !empty($_GET['menuname']) ? safe_replace($_GET['menuname']) : '';
...
//			}
		...
		if ($roleid == 1 && !empty($menuname)){
			$userlist = $this->pubdb->get_list("select a.userid from ho_sys_user_priv a,ho_sys_menu_new b where a.mid = b.mid and b.mname='$menuname'");
			if ($userlist){
				foreach ($userlist as $ulist){
					$userstr .= $ulist['userid'].',';
				}
				$userstr = substr($userstr,0,-1);
			}
			$rolelist = $this->pubdb->get_list("select DISTINCT(a.roleid) from ho_sys_user_role_priv a,ho_sys_menu_new b where a.mid = b.mid and b.mname='$menuname'");
			if ($rolelist){
				foreach ($rolelist as $rlist){
					$rolestr .= $rlist['roleid'].',';
				}
				$rolestr = substr($rolestr,0,-1);
			}
			$hasuserpriv = $this->pubdb->get_list("select distinct(userid) from ho_sys_user_priv");
			if ($hasuserpriv){
				foreach ($hasuserpriv as $privlist){
					$privstr .= $privlist['userid'].',';
				}
				$privstr = substr($privstr,0,-1);
			}
			$wherepriv = $userstr!='' ? "userid in ($userstr)" : "";
			$wherepriv .= $rolestr!='' ? $wherepriv!='' ? " or (roleid in ($rolestr) and userid not in ($privstr))" : "roleid in ($rolestr) and userid not in ($privstr)" : "";
			$wherestr = $wherepriv !='' ? $wherepriv : 'roleid=1';
		}
		
		$list = $this->userdb->get_list_page($wherestr, 'userid,username,realname,lastloginip,lastlogintime,logintimes,roleid,status,isrolepriv', 'status', $page, $this->pagesize);
		if ($recordnum <= 0) {
	...
	public function add(){
		if (isset($_POST['doSubmit'])) {
			...
			$info['realname'] = trim(safe_replace($_POST['realname']));
			...
			$roledb = common::load_model('role_model');
			$data['view'] = 'add';
			$roleid = intval($_SESSION['roleid'])==1?0:intval($_SESSION['roleid']);
			$data['rolelist'] = $roledb->select("roleid>$roleid", 'roleid,rolename');
			$data['roleid']   = isset($_GET['roleid']) ? intval($_GET['roleid']) : 0;

			$userid = intval($_GET['userid']);
			$data['info']  = array();
			if ($userid > 0){
				$data['info']  = $this->userdb->get_one(array('userid'=>$userid), 'userid,username,realname,email,lastlogintime,lastloginip,status,lang,roleid');			
			}
	...
	public function edit_password(){
		...
		if (isset($_POST['doSubmit'])){
			...
			$userinfo = $this->userdb->get_one(array('userid'=>$userid), 'userid,username,realname,email,lang');
			$data['info'] = $userinfo;
			unset($userinfo);
