...
class log extends admin {
	...
	private function get_log_list(){
		...
		$op = isset($_GET['op']) && !empty($_GET['op']) ? trim($_GET['op']) : 'operation';
		$content = trim(safe_replace($_GET['content']));
		...
		switch ($op) {
			...
			case 'source':
				...
				if ($_SESSION['roleid'] > 3) {
					...
					$customs = $userdb->select("roleid>3", 'userid, username');
					$wherestr .= !empty($wherestr) ? ' AND ' : '';
					if ($customs) {
						$customIds = array();
						foreach ($customs as $key => $value) {
							$customIds[] = $value['userid'];
						}
						$wherestr .= 'userid in ('.implode(',', $customIds).')';
					}else {
						$wherestr .= 'userid='.$_SESSION['userid'];
					}
				}
			...
			case 'error':
				...
				if ($op == 'mysql') {
					$file = CACHE_PATH.'log/'.$day.'_mysql_error_log.php';
				}else {
		...
		if (!empty($content)) {
			$wherestr = empty($wherestr) ? "content like '%$content%'" : $wherestr. " AND content like '%$content%'";
		}

		if (!in_array($op, array('error', 'mysql'))){
			...
			if ($op != 'active' && $op != 'cron' && $list) {
				foreach ($list as $key => $value) {
					$list[$key]['iparea'] = convertip($value['ip']);
				}
			}
		}
	...
	public function mysql(){
		$op = 'mysql';
		$data['filelist'] = $this->get_log_files(CACHE_PATH.'log/', 'mysql_error_log');
		include template('manage', 'log_error');
	}
	/**
	 * 旧后台操作日志
