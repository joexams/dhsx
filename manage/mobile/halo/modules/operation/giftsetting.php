...
class giftsetting extends admin {
	...
	public function ajax_list(){
		...
		$list = $this->pubdb->get_list_page($wherestr, '*', 'dateline DESC', $page, $pagesize);
	...
	public function add() {
		...
		if (!empty($soul)) {
			foreach ($soul['id'] as $key => $svalue) {
				...
				$soullist[] = array(
					...
					'number' => intval($soul['number'][$key]) > 0 ? intval($soul['number'][$key]) : 1,
				);
			}
		}
		...
		output_json(1, $msg);
			...
			$data['info'] = $giftarr;
			include template('ajax', 'giftsetting_add');
		}
	}
}
