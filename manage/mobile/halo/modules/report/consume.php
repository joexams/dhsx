...
class consume extends admin {
	...
	public function total(){
		if (isset($_GET['doSubmit'])) {
			...
			foreach ($sid as $value) {
				...
				if ($this->getdb !== false){
					...
					if ($start_level > 0 || $end_level > 0){
						if ($typeid > 0) {
							$sql = 'SELECT type, COUNT(DISTINCT a.player_id) AS num, COUNT(a.id) AS total, SUM(change_charge_value+value) AS ingot FROM player_ingot_change_record a 
						...
						$numsql = 'SELECT COUNT(DISTINCT a.player_id) AS allnum, COUNT(a.id) AS alltotal, SUM(change_charge_value+value) AS allingot FROM player_ingot_change_record a 
								LEFT JOIN player b ON a.player_id=b.id LEFT JOIN player_role c ON b.id=c.player_id AND b.main_role_id=c.id WHERE '.$wherestr;	
					}else {
						if ($typeid > 0) {
							$sql = 'SELECT type, COUNT(DISTINCT player_id) AS num, COUNT(a.id) AS total, SUM(change_charge_value+value) AS ingot FROM player_ingot_change_record a LEFT JOIN player b ON a.player_id=b.id WHERE '.$wherestr;
						}else {
							$sql = 'SELECT type, COUNT(DISTINCT player_id) AS num, COUNT(a.id) AS total, SUM(change_charge_value+value) AS ingot FROM player_ingot_change_record a LEFT JOIN player b ON a.player_id=b.id WHERE '.$wherestr.' GROUP BY type';
						}
						$numsql = 'SELECT COUNT(DISTINCT player_id) AS allnum, COUNT(a.id) AS alltotal, SUM(change_charge_value+value) AS allingot FROM player_ingot_change_record a LEFT JOIN player b ON a.player_id=b.id WHERE '.$wherestr;
					}
			...
			$this->getdb->table_name  = 'ingot_change_type';
			$typelist = $this->getdb->select('type=0', 'id,name');	
