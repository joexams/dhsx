...
class public_model extends model {
	 ...
	 * @return 文字模版内容
	 */
	public function get_text($sid,$id){
		$getdb = $this->set_db($sid);
		$info = $getdb->get_row("select text from chinese_text where id='$id'");
		return $info["text"];
	}
	 ...
	 * @return $player_id
	 */
	public function get_player_id($sid,$param){
		$getdb = $this->set_db($sid);
		$player = $getdb->get_row("select * from player where $param");
		return $player;
	}
}