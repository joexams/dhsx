...
function getsearchlogcrystalList(index){
	...
	$( "#list" ).fadeOut( "medium", function () {
		$.ajax({
			dataType: "json",
			url: query,
			data: $('#get_search_crystal_submit').serialize()+'&typeflag='+typeflag,
			success: showlogcrystalList
		});
	});
	return false;
}

function showlogcrystalList( data ) {
	if (data.status == 1){
		$('#list').html(data.msg);
	}else {
		if (data.type != undefined){
			typeflag = 1;
			typelist = data.type;
			$('#crystalstypetpl').tmpl( data.type.cons ).appendTo('#crystalscons');
			$('#crystalstypetpl').tmpl( data.type.get ).appendTo('#crystalsget');
		}
		
		recordNum = data.count;
		pageCount = Math.ceil( data.count/pageSize ), list = data.list;
		$( "#crystalspager" ).pager({ pagenumber: pageIndex, pagecount: pageCount, word: pageword, buttonClickCallback: getsearchlogcrystalList });
		$( "#list" ).empty();
...
$(document).ready(function(){
	if (typeof dialog != 'undefined'){
		dialog.title("<?php echo Lang('player').urldecode($_GET['title']).'->'.Lang('crystal_log'); ?>");
	}

	$('.h_lib_nav').on('click', 'a.crystals_type', function(){
		...
		if (typeid > 0) {
			if ($(this).parent('li').hasClass('hover') === true){
				...
				$('#get_search_crystal_submit').append('<input type="hidden" id="typeid_'+typeid+'" name="typeid[]" value="'+typeid+'">');
			}
		}
		return false;
	});

	$('#get_search_crystal_submit').on('submit', function(e){
		e.preventDefault();
		recordNum = 0;
		getsearchlogcrystalList(1);
	});

	$('#get_search_crystal_submit').submit();;
});
</script>
<script type="text/template" id="crystalstypetpl">
<li><a href="javascript:;" data-id="${id}" class="crystals_type">${name}</a><span></span></li>
</script>
...
<div id="bgwrap">
	...
	<br class="clear">
	<form id="get_search_crystal_submit" action="<?php echo INDEX; ?>?m=server&c=get&v=player_record" method="get" name="form">
		<div class="nav singlenav">
			...
			<li>
				<p>
					...
					<input type="hidden" name="key" value="crystal">
		...
		<table class="global" width="100%" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
				    ...
				    <th style="width:20%"><?php echo Lang('crystals') ?></th>
	...
	<div class="pagination" id="crystalspager"></div>
</div>
