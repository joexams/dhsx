...
function getsearchlogtake_bibleList(index){
	...
	$( "#list_"+pid ).fadeOut( "medium", function () {
		$.ajax({
			dataType: "json",
			url: query,
			data: $('#get_search_take_bible_submit').serialize()+'&typeflag='+typeflag,
			success: showlogtake_bibleList
		});
	});
	return false;
}
...
function showlogtake_bibleList( data ) {
	if (data.status == 1){
		...
		if (data.type != undefined){
			if (typeflag == 0){
				typelist = data.type;
				$('#take_bibletypetpl').tmpl( data.type.cons ).appendTo('#take_biblecons');
				$('#take_bibletypetpl').tmpl( data.type.get ).appendTo('#take_bibleget');
			}else {
				typelist.players = data.type.players;
			}
			typeflag = 1;
		}
		...
		$( "#take_biblepager" ).pager({ pagenumber: pageIndex, pagecount: pageCount, word: pageword, buttonClickCallback: getsearchlogtake_bibleList });
		$('#list_'+pid).empty();
		if (data.count > 0){
...
$(document).ready(function(){
	if (typeof dialog != 'undefined'){
		dialog.title("<?php echo Lang('player').urldecode($_GET['title']).'->'.Lang('take_bible_log'); ?>");
	}

	$('.h_lib_nav').on('click', 'a.take_bible_type', function(){
		...
		if (typeid > 0) {
			if ($(this).parent('li').hasClass('hover') === true){
				...
				$('#get_search_take_bible_submit').append('<input type="hidden" id="typeid_'+typeid+'" name="typeid[]" value="'+typeid+'">');
			}
		}
		return false;
	});

	$('#get_search_take_bible_submit').on('submit', function(e){
		e.preventDefault();
		recordNum = 0;
		getsearchlogtake_bibleList(1);
	});

	$('#get_search_take_bible_submit').submit();
});
</script>
<script type="text/template" id="take_bibletypetpl">
<li><a href="javascript:;" data-id="${id}" class="take_bible_type">${name}</a><span></span></li>
</script>
...
<div id="bgwrap">
	<form id="get_search_take_bible_submit" action="<?php echo INDEX; ?>?m=server&c=get&v=player_record" method="get" name="form">
	<div class="h_lib_nav">
	<ul id="take_biblecons">
	</ul>
	<ul id="take_bibleget">
	</ul>
	</div>
		...
		<ul class="nav_li">
			...
			<li class="nobg">
				<p>
					...
					<input type="hidden" name="key" value="take_bible">
	...
	<div class="content">
		<table class="global" width="100%" style="max-width:800px;min-width:600px;" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
				    ...
				    <th style="10%"><?php echo Lang('take_bible_npc'); ?></th>
		...
		<div class="pagination" id="take_biblepager"></div>
	</div>
</div>
