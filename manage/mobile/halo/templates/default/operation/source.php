...
$(function(){
	...
	<?php if ($data['key'] == 'delete_role') { ?>
		$('#player').on('blur', function() {
			var player = $.trim($(this).val());
			var cid = $('#cid').val(), sid = $('#sid').val();
			var player_type = $('#player_type').val();
			if (player != '' && cid > 0 && sid > 0) {
				var url = '<?php echo INDEX; ?>?m=server&c=get&v=role_list';
				Ha.common.ajax(url, 'json', {sid: sid, player: player, player_type: player_type}, 'get', 'container', function(data){
					$('label', $('#player_role')).remove();
					if (data.status == 0){
						if (data.count > 0){
							$( "#player_role" ).empty();
							$( "#rolelisttpl" ).tmpl( data.list ).appendTo( "#player_role" );
						}
					}else {
						$('label', $('#player_role')).remove();
					}
...
<script type="text/template" id="rolelisttpl">
	<label style="position:absolute;left:350px;"><input type="checkbox" name="player_role_ids[][id]" value="${id}"> ${name}</label><br>
</script>
<?php } ?>
<?php if ($data['key'] == 'delete_item') { ?>
