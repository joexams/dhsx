...
function showStatData( data ) {
	...
	if (data.count > 0){
		...
		var ssid = ccid = combined_to = 0, sname = ''; 
		...
		for(var key in pserverlist){
			...
			combined_to = pserverlist[key].combined_to;
			if (ssid > 0 && ccid > 0 && pserverlist[key].api_server != '000' && pserverlist[key].opendate<=data.todaytime){
				 newQueue.offer({
					...
					data: {cid: ccid, sid: ssid, name: sname, combined_to: combined_to, rnd: Math.random()},
					dataType: 'json',
...
<tr id="s_${sid}">
   ...
   <?php if ($roleid <= 3) { ?>
	...
	{{else}}
	<a href="#app=4&cpp=35&url=${encodeurl('report', 'gamereport', 'player','&sid=')}${sid}%26cid%3D${cid}%26title%3D${encodeURI(name!=''? name : o_name)}" class="playerreport"><?php echo Lang('report') ?></a>
	{{/if}}
	</td>
	<td class="online">--</td>
	<td class="active">--</td>
