...
function backpack_type_id_to_name(id){
		if (extendlist.backpack_type != undefined && extendlist.backpack_type.length > 0){
		for (var key in extendlist.backpack_type){
			if (extendlist.backpack_type[key].id == id){
				return textid_to_name(extendlist.backpack_type[key].name_text_id);
			}
		}
	}
	return '';
...
function item_id_to_name(id){
		...
		for (var key in extendlist.item){
			if (extendlist.item[key].id == id){
				return textid_to_name(extendlist.item[key].name_text_id);
			}
		}
	}
	return '';
}
function skill_id_to_name(id){
		if (extendlist.skill_base != undefined && extendlist.skill_base.length > 0){
		for (var key in extendlist.skill_base){
			if (extendlist.skill_base[key].id == id){
				return textid_to_name(extendlist.skill_base[key].name_text_id);
			}
		}
	}
	return '';
...
<tr>
<td class="num">${id}</td>
<td>${backpack_type_id_to_name(backpack_type_id)}</td>
<td>${item_id_to_name(item_id)}</td>
<td>${attack}</td>
<td>${defense}</td>
<td>${health}</td>
<td>${durable}</td>
<td>${skill_id_to_name(item_skill_id)}</td>
</tr>
...
<div class="column cf" id="table_column">
	...
	<tr id="dataTheadTr">
        ...
        <th><?php echo Lang('attack'); ?></th>
        <th><?php echo Lang('defense'); ?></th>
	    <th><?php echo Lang('health'); ?></th>
	    <th><?php echo Lang('durable'); ?></th>
	    <th><?php echo Lang('ability'); ?></th>
	</tr>
	<?php }else if ($key == 'fate') { ?>
	<tr id="dataTheadTr">
	    <th>&nbsp;</th>
