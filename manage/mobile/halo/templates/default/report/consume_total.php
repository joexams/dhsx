...
$(function(){
	...
	$('#get_submit').on('submit', function(e){
		...
		if (cid > 0 && sid != null){
			var url = '<?php echo INDEX; ?>?m=report&c=consume&v=total';
			Ha.common.ajax(url, 'json', objform.serialize(), 'get', 'container', function(data){
				...
				if (data.status == 0){
					...
					$('#consumelist').html('<tr><td colspan="4" style="text-align: left"><?php echo Lang('no_find_data')?></td></tr>');
				}
			}, 1);
		}else {
...
<h2><span id="tt"><?php echo Lang('consume_total'); ?></span></h2>
	...
	<div class="frm_cont" id="submit_area">
	    ...
	    <ul>
	        ...
	        <li>
	            ...
	            <select name="typeid" id="typeid" class="ipt_select">
