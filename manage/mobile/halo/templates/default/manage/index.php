...
<div id="header">
	<div class="logo">
		<a href=<?php echo "./".INDEX;?> title="<?php echo Lang('da_hua_shen_xian')?>"><img src="static/images/logo.png"  alt="<?php echo Lang('da_hua_shen_xian')?>" /></a>
	</div>
	<div class="nav_cont">
		<div class="menunav">
			<ul id="mainmenu"></ul>
...
<div class="dashboard cf" id="wrap_common">
	...
	<div id="main" class="main">
		...
		<div id="footer">
			...
			<p>
	        	<a onclick="window.scroll(0,0);return false" title="<?php echo Lang('back_to_top')?>" href="#header" class="gotop">T</a>
	        </p>
		</div>
	</div>
</div>
...
<div>
	...
	<div id="js_N_nav_footer_trigger" class="sitemap-ctrl sitemap-ctrl-active" style="position: fixed;">
		<?php echo Lang('easy_to_navigate_menu')?><i class="i_orderd"></i>
	</div>
    <div id="js_N_nav_footer" class="fixed-sitemap js_N_nav_footer_nav_fixed" style="bottom: 0px;">
	    <div class="fixed-sitemap-main">
	        <div class="sitemap-flink" id="quickguidelist">
	            <a href="javascript:;" id="add_quick" title="<?php echo Lang('add').Lang('easy_to_navigate_menu')?>" style="margin-left:30px;"><span><i class="i_add">+</i><?php echo Lang('add')?></span></a>
	        </div>
	    </div>
    </div>
    </div>
...
var global_companylist = [], global_serverlist = [], global_text_info = [];
...
$(function(){
	...
	$('#quickguidelist').on('click', 'a#add_quick', function(){
		...
		Ha.common.ajax(url, 'html', queryData, 'get', 'container', function(data){
			Ha.Dialog.show(data, '<?php echo Lang('add_shortcut')?>', 500, 'quickguideDlg');
		}, 1);
	});
...
global_text_info = <?php echo $text_info ? $text_info : '[]'; ?>;
function textid_to_name(textid) {
		if (typeof global_text_info != 'undefined') {
			for(var key in global_text_info) {
				if (global_text_info[key].id == textid) {
					return global_text_info[key].text;
				}
			}
		}
		return '-';
