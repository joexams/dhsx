		...
		for (var i=0; i<sids.length; i++) {
			for (var key in global_serverlist) {
				if (global_serverlist[key].sid == sids[i]) {
					names += global_serverlist[key].name+' ';
					break;
				}
			}
		}
...
function getServerByCid(cid) {
    ...
    var server = [];
    for (var key in global_serverlist) {
        if (global_serverlist[key].cid == cid  && global_serverlist[key].combined_to==0 && global_serverlist[key].open==1) {
