			...
			if (playername != '' && cid > 0 && sid > 0) {
				...
				Ha.common.ajax(url, 'json', {sid: sid, cid: cid, key: 'item', playername: playername, player_type: player_type}, 'get', 'container', function(data){
					...
					if (data.status == 0){
						...
						if (data.count > 0){
							$( "#itemlisttpl" ).tmpl( data.list ).prependTo( "#player_item" );
						}
					}else {
						$('label', $('#player_item')).remove();
					}
...
<script type="text/template" id="fatelisttpl">
	<label><input type="checkbox" name="player_fate_ids[][player_fate_id]" value="${id}"> {{html fate_id_to_name(fate_id, fate_level)}}</label>
</script>
<?php } ?>
<?php if ($data['key'] == 'delete_item') { ?>
<script type="text/template" id="itemlisttpl">
	<label><input type="radio" name="player_item_id" value="${id}"> {{html item_id_to_name(item_id)}}</label>
</script>
<?php } ?>
