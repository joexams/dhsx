	...
	$('#selectOrder').on('click', 'a.logtype', function(){
		...
		if (typeid > 0) {
			...
			if (key != ''){
		            ...
		            var queryData = $('#get_search_submit').serialize()+'&sid=<?php echo $data['sid'] ?>&id=<?php echo $data['id']; ?>&version=<?php echo $data['version']; ?>&key='+key;
		            $('.active', $('#selectOrder')).removeClass('active');
		            $(this).addClass('active');
        	...
        	<div class="frm_cont">
				<ul>
					<li name="condition" id="selectOrder">
					    ...
					    <a href="javascript:;" class="logtype" data-id="<?php echo $block['bid']; ?>" data-key="<?php echo $block['key']; ?>"><?php echo $block['bname']; ?></a>
					    <?php } ?>
					</li>
				</ul>
			</div>
