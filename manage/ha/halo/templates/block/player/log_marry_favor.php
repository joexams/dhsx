...
var pageIndex = 1, pageCount = 0, pageSize = 20, recordNum = 0, typeid = 0, typeflag = 0, list, typelist;
function getsearchlogmarry_favorList(index){
	...
	$( "#list" ).fadeOut( "medium", function () {
		$.ajax({
			dataType: "json",
			url: query,
			data: $('#get_search_marry_favor_submit').serialize()+'&typeflag='+typeflag,
			success: showlogmarry_favorList
		});
	});
	return false;
}

function showlogmarry_favorList( data ) {
	if (data.status == 1){
		$('#list').html(data.msg);
	}else {
		if (data.type != undefined){
			typeflag = 1;
			typelist = data.type;
			$('#marry_favorstypetpl').tmpl( data.type.cons ).appendTo('#marry_favorscons');
			$('#marry_favorstypetpl').tmpl( data.type.get ).appendTo('#marry_favorsget');
		}
		
		recordNum = data.count;
		pageCount = Math.ceil( data.count/pageSize ), list = data.list;
		$( "#marry_favorspager" ).pager({ pagenumber: pageIndex, pagecount: pageCount, word: pageword, buttonClickCallback: getsearchlogmarry_favorList });
		$( "#list" ).empty();
...
$(document).ready(function(){
	if (typeof dialog != 'undefined'){
		dialog.title("<?php echo Lang('player').urldecode($_GET['title']).'->'.Lang('marry_favor_log'); ?>");
	}

	$('.h_lib_nav').on('click', 'a.marry_favors_type', function(){
		...
		if (typeid > 0) {
			if ($(this).parent('li').hasClass('hover') === true){
				...
				$('#get_search_marry_favor_submit').append('<input type="hidden" id="typeid_'+typeid+'" name="typeid[]" value="'+typeid+'">');
			}
		}
		return false;
	});

	$('#get_search_marry_favor_submit').on('submit', function(e){
		e.preventDefault();
		recordNum = 0;
		getsearchlogmarry_favorList(1);
	});

	$('#get_search_marry_favor_submit').submit();;
});
</script>
<script type="text/template" id="marry_favorstypetpl">
<li><a href="javascript:;" data-id="${id}" class="marry_favors_type">${name}</a><span></span></li>
</script>
...
<div id="bgwrap">
	...
	<div class="h_lib_nav">
	<ul id="marry_favorsget">
	</ul>
	</div>
	<br class="clear">
	<form id="get_search_marry_favor_submit" action="<?php echo INDEX; ?>?m=server&c=get&v=player_record" method="get" name="form">
		<div class="nav singlenav">
			<ul class="nav_li">
				...
				<p>
					...
					<?php echo Lang('marry_favors') ?>
					<input type="text" name="marry_favors" value="0">
				</p>
			</li>
			<li>
				<p>
					...
					<input type="hidden" name="key" value="marry_favor">
					<input type="hidden" name="typeflag" value="1">
					<input type="submit" name="getsubmit" id="get_search_submit" value="搜索" class="button_link">
				</p>
			</li>
				<li class="nobg">
					<p>获取类：<span class="orangetitle" id="getnum">0</span></p>
				</li>
				<li class="nobg">
					<p>消费类：<span class="greentitle" id="connum">0</span></p>
				</li>
			</ul>
		</div>
	</form>
		...
		<table class="global" width="100%" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
				    ...
				    <th style="width:20%"><?php echo Lang('marry_favors') ?></th>
	...
	<div class="pagination" id="marry_favorspager"></div>
</div>
