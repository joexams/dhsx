		...
		if ($cid > 0){
			...
			if (count($gamelist) > 0) {
				foreach ($gamelist as $key => $value) {
					...
					$gamelist[$key]['url'] = '#app=5&cpp=56&url='.urlencode(WEB_URL.INDEX.'?m=report&c=data&v=servertotal&cpp=56&sid='.$value['sid'].'&name=');
...
*/
	...
	public function compare(){
		...
		$endTime = strtotime($month.'-'.str_pad($day_num, 2, '0', STR_PAD_LEFT));
	...
	public function ajax_compare_list(){
		...
		unset($listday, $listday1, $listday2, $chartData);
