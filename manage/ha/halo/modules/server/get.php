...
HTML;
	...
	public function player_record(){
		...
		$key_array = array('coin', 'ingot', 'soul', 'soul_stone', 'item', 'fame', 'fate', 'power', 'take_bible', 'skill', 'farmland', 'flower_count', 'coin_tree_count', 'elixir', 'state_point', 'peach', 'defeat_world_boss', 'level_up', 'mission', 'item_attribute_stone', 'faction_contribution', 'spirit', 'xian_ling', 'xianling_tree', 'long_yu_ling', 'deploy_start', 'crystal', 'marry_favor');
	...
	public function player_townonline()
	{
		...
		foreach ($list as $key => $value) {
			...
			$count = $this->apiadmin->get_town_player_count($value['id']);
			$town_list[$i]['player_count'] = $count ? $count['player_count'] : 0;
			$town_list[$i]['rank'] = $i+1;
			$i++;
		}

		$onlie = $this->apiadmin->get_all_town_player_count();
	...
	public function player_asset()
	{
		...
		if($section){
			$data['list']['ingot'] = array(
				...
				array('name' => '500001 - ∞', 'name' => '101 - 500', 'player' => $section['ingot_player_9'],'num' => $section['ingot_num_9'])
			);
	...
	private function player_key_record($tbl_key, $get){
		...
		if (!empty($typeid)){
			$type = in_array($tbl_key, array('fame', 'fate', 'power', 'take_bible', 'skill', 'coin_tree_count', 'long_yu_ling', 'xian_ling', 'xianling_tree', 'crystal', 'marry_favor')) ? 'op_type' : 'type';
		...
		if ($coins > 0) {
			$wherestr .= empty($wherestr) ? "value={$coins}" : " AND value={$coins}";
		}
		if ($starttime > 0){
		...
		if (in_array($tbl_key, array('fame', 'fate', 'power', 'take_bible', 'skill', 'farmland', 'flower_count', 'coin_tree_count', 'elixir', 'item_attribute_stone', 'faction_contribution', 'spirit','xian_ling', 'xianling_tree', 'long_yu_ling', 'crystal', 'marry_favor'))){
		...
		if ($tbl_key == 'marry_favor') {
			$sql = "SELECT * FROM {$table_name} WHERE $wherestr ORDER BY {$order} LIMIT $offset, $pagesize";
		}else {
			$sql = "SELECT username, nickname, a.* FROM {$table_name} a INNER JOIN player b ON a.player_id=b.id WHERE $wherestr ORDER BY {$order} LIMIT $offset, $pagesize";
		}
		$list = $this->getdb->get_list($sql);
...
//		return array('list'=>$list, 'count'=>$recordnum, 'allnum'=>$allnum);
		return array('list'=>$list, 'count'=>$recordnum);
	}
	/**
	 * 玩家游戏信息
	 * 
	...
	private function player_key_record_type($tbl_key){
		...
		if (in_array($tbl_key, array('fame', 'fate', 'power', 'take_bible', 'skill', 'coin_tree_count', 'crystal', 'marry_favor')) ){
			$this->getdb->table_name = $tbl_key.'_log_type';
		}else if (in_array($tbl_key, array('faction_contribution', 'xianling_tree'))) {
