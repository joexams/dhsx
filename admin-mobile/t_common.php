...
function  Common()
{
	...
	$table = KillBad('table') == '' ? 'town' : KillBad('table'); //表名
	...
	if (!empty($search_column) && ReqStr("$search_column")){
		$search_column_value = ReqStr("$search_column");
		$search_column_value_id = chinese_text("$search_column_value");
		$where = "where $search_column = '$search_column_value_id'";
		$page_param = "&$search_column=$search_column_value";
	}
	//获取总数量
...
function  SetCommon()
{
		...
		//循环所有的列
		{
			...
			if (in_array("$id_old[$i]",$column_arr)){
				...
				foreach ($table_Columns_list as $list)
				{
					...
					$is_template = $list['is_template']; //文字模版
					switch ($list['column_type']) //字段类型
					{
						...
						default:
							...
							$field_value = $$list['column_name'];
							$is_template == 1 ? $k_value = chinese_text("$field_value[$i]") : $k_value = $field_value[$i];
							$update_field .= "`$field` = '$k_value',";//更新字段
					}
				}
				//存在更新的字段则执行SQL
				if ($update_field)
	...
	foreach ($table_Columns_list as $list)
	{
		...
		{
			case 'int':
				$list['is_template'] == 1 ? $$field_n = ReqStr("$field_n") : $$field_n = ReqNum("$field_n");
				break;
			case 'float':
				$$field_n = ReqFloat("$field_n");
			default:
		...
		//变量值
		$value_n =$$field_n;
		$list['is_template'] == 1 ? $value_n = chinese_text("$value_n") : '';
	...
	if ($_FILES["fileField"]["name"]){
		...
		$PHPExcel = $objReader->load($_FILES["fileField"]["tmp_name"]);
		...
		for ($row=1;$row<=$allRow;$row++){
		    ...
		    for ($col=0; $col<$allCol;$col++){
		    	$value_excel = $sheet->getCellByColumnAndRow($col,$row)->getValue();
		        if (preg_match("/[\x7f-\xff]/", $value_excel)) { 
					$value_excel=chinese_text("$value_excel"); 
				}
				trim($value_excel)!="" ? $value_n_str_excel .="'$value_excel'," : '';
		    }
