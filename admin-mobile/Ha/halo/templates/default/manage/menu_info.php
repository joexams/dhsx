...
<div class="column">
	...
	<table>
		<thead>
			<tr>
				<th>序号</th><th>字段</th><th>字段类型</th><th>字段描述</th><th>类型</th><th>类型框长度</th><th>默认值</th><th>字段关联SQL语句</th><th>类型固定值</th><th>是否开启</th><th>排序</th><th>选择搜索</th><th>主键(必填)</th><th>文字模版</th><th>是否搜索</th>
			</tr>
		...
		<?php foreach ($column_list as $key => $value){?>
			<tr id='<?php echo $value["id"]?>'>
				...
				<td id='is_template'>
				<select name='is_template<?php echo $value["id"]?>' onchange='modify(this)'>
				<option value='1' <?php if ($value["is_template"]==1) echo "selected"?>>是</option>
				<option value='0' <?php if ($value["is_template"]==0) echo "selected"?>>否</option>
				</select>
				</td>
				<td id='is_search'>
				<select name='is_search<?php echo $value["id"]?>' onchange='modify(this)'>
				<option value='1' <?php if ($value["is_search"]==1) echo "selected"?>>是</option>
				<option value='0' <?php if ($value["is_search"]==0) echo "selected"?>>否</option>
				</select>
				</td>
			</tr>
			<?php }?>
