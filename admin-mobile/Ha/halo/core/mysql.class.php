...
//		echo $sql;
	...
	public function halt($message = '', $sql = '') {
		$this->errormsg = "<b>MySQL Query : </b> $sql <br /><b> MySQL Error : </b>".$this->error()." <br /> <b>MySQL Errno : </b>".$this->errno();
		$msg = $this->errormsg;
	    echo '<div style="font-size:12px;text-align:left; border:1px solid #9cc9e0; padding:1px 4px;color:#000000;font-family:Arial, Helvetica,sans-serif;"><span>'.$msg.'</span></div>';
		//echo '<div>MySQL Error'.$this->errno().'<br/></div>';
		$msg = $this->error();
		$sql = str_replace(array("\r\n", "\n", "\r"), '', $sql);
		$msg = str_replace(array("\r\n", "\n", "\r"), '', $msg);
		error_log("<?php exit;?>\t".date('Y-m-d H:i:s',TIME)."\t".$this->errno()."\t".$msg."\t".$sql.PHP_EOL, 3, CACHE_PATH.'log/'.date('Ym').'_mysql_error_log.php');
		exit;
	}
