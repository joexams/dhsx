...
function MissionAreaMonsterTeam()
{
	...
	//代表怪物
	$query = $db->query('select a.id,b.text as name from role a,chinese_text b where a.name_text_id=b.id');
	while($rs = $db->fetch_array($query)){
		$monster_list[] =  $rs;
	}
	//副本区域
	$query = $db->query('select a.id,concat(c.text,a.number) as name from mission_area a,mission b,chinese_text c where a.mission_id=b.id and b.name_text_id=c.id');
	while($rs = $db->fetch_array($query)){
...
function Monster()
{
	...
	//角色
	$query = $db->query('select a.id,concat(a.xin_ji,"星",a.jue_xing,"觉",b.text) as name,a.first_skill,a.second_skill,a.third_skill,a.fourth_skill from role a,chinese_text b where a.name_text_id=b.id order by a.name_text_id');
	while($rs = $db->fetch_array($query)){
		$role_list[] =  $rs; 
	...
	//技能
	$query = $db->query('select a.skill_number,b.id,CONCAT(c.text,b.lv) as name,a.id as skill_id from skill_base a,skill_lv b,chinese_text c where a.id=b.skill_base_id and a.name_text_id=c.id');
	while($rs = $db->fetch_array($query)){
...
function  SetMonster() 
{
	...
	if ($id_old)
	{
		...
		for ($i=0;$i<=$id_num;$i++)	
		{
			if ($id_old[$i])
			{
				$attack = $role_list[$role_id[$i]]["attack"]+($role_list[$role_id[$i]]["attack_add"]*$role_lv[$i])+$attack_add[$i];
				$defense = $role_list[$role_id[$i]]["defense"]+($role_list[$role_id[$i]]["defense_add"]*$role_lv[$i])+$defense_add[$i];
				$health = $role_list[$role_id[$i]]["health"]+($role_list[$role_id[$i]]["health_add"]*$role_lv[$i])+$health_add[$i];
				$db->query("
				update 
					monster 
				set 
	...
	if ($role_id_n>0)
	{
		$attack = $role_list[$role_id_n]["attack"]+($role_list[$role_id_n]["attack_add"]*$role_lv_n)+$attack_add_n;
		$defense = $role_list[$role_id_n]["defense"]+($role_list[$role_id_n]["defense_add"]*$role_lv_n)+$defense_add_n;
		$health = $role_list[$role_id_n]["health"]+($role_list[$role_id_n]["health_add"]*$role_lv_n)+$health_add_n;
		$query = $db->query("
		insert into monster			(`id`,`role_id`,`role_lv`,`sign`,`attack_add`,`defense_add`,`health_add`,`attack`,`defense`,`health`,`first_skill_lv_id`,`second_skill_lv_id`,`third_skill_lv_id`,`fourth_skill_lv_id`) 
		values 			('','$role_id_n','$role_lv_n','$sign_n','$attack_add_n','$defense_add_n','$health_add_n','$attack','$defense','$health','$first_skill_lv_id_n','$second_skill_lv_id_n','$third_skill_lv_id_n','$fourth_skill_lv_id_n')
		");
		if($query)
		{
			$msg .= "<br />增加成功！";
