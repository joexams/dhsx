...
if (empty($openid) || empty($appid) || $ts <= 0 || empty($contractid) || empty($sig) || empty($payitem) || empty($billno) || 2 != $providetype)	common::output(1000, '参数错误');
if (common::isOpenId($openid) === false)	common::output(1000, 'openid参数错误');
//if ('' != $contractid)	common::output(1000, 'contractid参数错误');

$verify = SnsSigCheck::verifySig('get', '/task/provide_award.php', $_GET, $appkey.'&', $sig);
