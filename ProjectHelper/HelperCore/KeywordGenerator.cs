...
namespace HelperCore
{
    public class KeywordGenerator
    {
        ...
        /// <param name="fileOfWords">词库文件路径</param>
        /// <param name="isReplace">是否生成替换关键字方法</param>
        /// <param name="isFullMatch">是否全字匹配</param>
        /// <returns></returns>
        public static string BuildErlangCodes(string moduleName, string fileOfWords, bool isReplace, bool isFullMatch)
        {
            string contentOfKeywords;

            using (StreamReader sr = new StreamReader(fileOfWords))
            {
            ...
            if(isFullMatch)
                ...
                strbldOfErlangCodes.Append("\t\t{ok, _, _} -> true\n");
            strbldOfErlangCodes.Append("\tend.\n\n");
            ...
            foreach (KeyValuePair<int, List<string>> keywordItem in dictOfKeywords)
            {
                ...
                if (keyword.Length > 1)
                {
                    ...
                    build(keyword, itemKeywords, 0, ref tempCondition);
        ...
        private static void build(string keyword, List<string> keywords, int index, ref StringBuilder output)
        {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (lengthOfSkey - index3 > 0)
                            {
                                ...
                                build(skey, tempKeywords, index2, ref output);
                            }
                            else
                            {
