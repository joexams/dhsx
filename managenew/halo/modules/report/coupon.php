...
class coupon extends admin {
	...
	public function active() {
		if (isset($_GET['dogetSubmit'])){
			...
			$and = !empty($wherestr) ? ' AND ' : '';
			$wherestr .= $usetype > 0 ? $and."ctime>0" : '';
			$and = !empty($wherestr) ? ' AND ' : '';
	...
	public function add_active() {
		...
		$this->extenddb->db_tablepre = '';
		$tbl_exists =  $this->extenddb->table_exists($table_name);
		if ($tbl_exists) {
		...
		$createsql = "CREATE TABLE `{$table_name}` (
			...
			`times`  tinyint(2) UNSIGNED NOT NULL DEFAULT 1 ,
			PRIMARY KEY (`id`),
		...
		$ret = $this->extenddb->query($createsql);
		if ($ret) {
			//直接生成兑换券
			if ($num > 0) {
	...
	public function export_active() {
		...
		$this->extenddb->table_name = $table_name;
		$list = $this->extenddb->select($wherestr, 'code');
		...
		foreach ($list as $key => $value) {
			echo $tbl_suffix.'_'.$value['code'].PHP_EOL;
		}
	}
	/**
	 * 生成兑换券
	...
	private function import_active($cid, $table_name, $num, $flag = false) {
		...
		//直接生成兑换券
		$this->extenddb->table_name = $table_name;
		$max = $this->extenddb->get_one('', 'MAX(times) AS times');
		$info = array();
		$cid = $cid;
		...
		for ($i=1; $i<=$n; $i++) {
			...
			$sql = "INSERT INTO $table_name(cid, times, code) VALUES".implode($values, ',');
