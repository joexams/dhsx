...
function getUserList(index) {
	var query = "<?php echo INDEX; ?>?m=manage&c=user&v=ajax_list&roleid=5&top="+index+"&recordnum="+recordNum;
	pageIndex = index;
	$( "#customerlist" ).fadeOut( "medium", function () {
		$.ajax({
			dataType: "json",
			url: query,
...
function showUserList( data, type) {
	if (data.status == -1){
		...
		$( "#customerlist" ).empty();
		if (data.count > 0){
			$( "#customerlisttpl" ).tmpl( data.list ).prependTo( "#customerlist" );
			$( "#customerlist" ).stop(true,true).hide().slideDown(400);
		}
	}
}
...
<div id="bgwrap">
	<ul class="dash1">
		<li class="fade_hover selected"><a href="javascript:;"><span><?php echo Lang('customer_manage'); ?></span></a></li>
	</ul>
	<br class="clear">
	<!-- 添加用户 -->
	<div class="onecolumn">
		...
		<div class="content" id="submit_area" style="display: none;">
			...
			<form name="post_user_submit" id="post_user_submit" action="<?php echo INDEX; ?>?m=manage&c=user&v=add" method="post">
				...
				<table class="global" width="100%" cellpadding="0" cellspacing="0">
					<tbody>
					<tr>
						<th style="width: 10%;"><?php echo Lang('username'); ?>：</th>
						<td style="width: 20%"><input type="text" name="username" id="username" style="width:90%"></td>
...
<br class="clear">
	<div class="content" id="submit_area">
		...
		<table class="global" width="100%" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
					<th style="width:10%;"><?php echo Lang('login_account'); ?></th>
