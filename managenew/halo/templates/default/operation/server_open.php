...
function getList(index){
	var query = "<?php echo INDEX; ?>?m=develop&c=server&v=server_list&cid=-1&top="+index+"&recordnum="+recordNum;
	pageIndex = index;
...
$(function(){
	...
	$('#post_submit').on('submit', function(e){
		...
		if (cid > 0 && name != '' && server != '') {
			...
			$('#list_op_tips').children('p').html('请填写输入框的每个字段!');
			$('#list_op_tips').fadeIn();
			setTimeout( function(){
				$('#list_op_tips').fadeOut();
				$('#btnsubmit').removeAttr('disabled');
		...
		<table class="global" width="100%" cellpadding="0" cellspacing="0">
			...
			<tbody>
				<tr>
					...
					<td><input type="text" name="o_name" id="o_name" value="大话神仙s"></td>
