...
function getsearchlogpeachList(index){
	...
	$( "#list" ).fadeOut( "medium", function () {
		$.ajax({
			dataType: "json",
			url: query,
			data: $('#get_search_peach_submit').serialize(),
			success: showlogpeachList
		});
	});
	return false;
}

function showlogpeachList( data ) {
	if (data.status == 1){
		$('#list').html(data.msg);
	}else {
		...
		$( "#peachpager" ).pager({ pagenumber: pageIndex, pagecount: pageCount, word: pageword, buttonClickCallback: getsearchlogpeachList });
...
$(document).ready(function(){
	if (typeof dialog != 'undefined'){
		dialog.title("<?php echo Lang('player').urldecode($_GET['title']).'->'; ?>"+'<?php echo Lang('peach').Lang('log'); ?>');
	}

	$('.h_lib_nav').on('click', 'a.peach_type', function(){
		...
		if (typeid > 0) {
			if ($(this).parent('li').hasClass('hover') === true){
				...
				$('#get_search_peach_submit').append('<input type="hidden" id="typeid_'+typeid+'" name="typeid[]" value="'+typeid+'">');
			}
		}
		return false;
	});

	$('#get_search_peach_submit').on('submit', function(e){
		e.preventDefault();
		recordNum = 0;
		getsearchlogpeachList(1);
	});

	$('#get_search_peach_submit').submit();
});
</script>
<script type="text/template" id="peachtypetpl">
<li><a href="javascript:;" data-id="${id}" class="peach_type">${name}</a><span></span></li>
</script>
...
<div id="bgwrap">
	<form id="get_search_peach_submit" action="<?php echo INDEX; ?>?m=server&c=get&v=player_record" method="get" name="form">
		...
		<ul class="nav_li">
			...
			<li class="nobg">
				<p>
					...
					<input type="hidden" name="key" value="peach">
	...
	<div class="content">
		<table class="global" width="100%" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
				    ...
				    <th style="width:80px;"><?php echo Lang('plus_exp'); ?></th>
				    <th>&nbsp;</th>
				</tr>
			</thead>
			<tbody id="list">
		...
		<div class="pagination" id="peachpager"></div>
	</div>
