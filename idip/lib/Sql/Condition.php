<?php 
class Sql_Condition implements Countable
{
	const COLUMN      = 0x0;
	const OPERATOR    = 0x1;
	const VALUE       = 0x2;
	...
	public function add($column, $operator, $value, $conjunction = 'AND', $type = 0x1)
	{
		...
		{
			throw new Exception('Invalid arithmetic operator (allowed: ' . implode(', ', self::$a_op) . ')');
		}

		if(!in_array($conjunction, self::$l_op))
		{
			throw new Exception('Invalid logic operator (allowed: ' . implode(', ', self::$l_op) . ')');
		}
	 ...
	 *
	 * @param $condition Sql_Condition
	 * @return Sql_Condition
	 */
	public function merge(Sql_Condition $condition)
	{
