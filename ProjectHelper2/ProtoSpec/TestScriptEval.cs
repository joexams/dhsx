...
namespace ProtoSpec
{
    ...
    public class TestScriptRunner
    {
        ...
        public static void Clean(TestScriptEvalContext context, int delayMilliSecond)
        {
            ...
            foreach (ProtoSpecColumn column in format.Columns)
            {
                switch (column.ColumnType)
                {
                    ...
                    case ProtoSpecColumnType.TypeOf:
                        {
                            TestScriptValue value = TestScriptValue.CreateObject();

                            ParseResponse(module, data, ref parseOffset, column.Format, value);

                            recvValue.AddProperty(column.Name, value);
                        }
                        break;
                }
            }
