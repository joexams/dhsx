...
class api_admin extends api_base {
	...
	const ADMIN_API_CHANGE_AD = 71;		//商城广告图片切换
	const ADMIN_API_INCREASE_CON = 72;	//增加玩家帮贡
	const ADMIN_API_INCREASE_XIAN_LING = 73;	//增加玩家仙令
	const ADMIN_API_READ_BACKUP = 74;	//回档
	const ADMIN_API_DELETE_ITERM = 75;	//删除物品
	const ADMIN_API_GIVE_SACRIFICE_COUNT = 76; 	//赠送上供次数
    const ADMIN_API_GIVE_DRAGONBALL = 77;		//赠送玩家龙珠
    const ADMIN_API_GET_PLAYER_ATTACK_POWER = 78;		//获取玩家战力
    const ADMIN_API_TEAM_BUYING_ACTIVITY_AWARD = 79;		//全网团购活动返还
	const ADMIN_API_PUBLISH_HIRE_ROLE = 80;     //发布雇佣伙伴
	const ADMIN_API_INCREASE_BA_XIAN_LING = 81;     //赠送八仙令
	const ADMIN_API_INCREASE_PEARL = 82;     //赠送元神珠
    const ADMIN_API_BUCHANG = 83;   // 发放补偿
    const ADMIN_API_GIVE_PLAYER_TITLE = 84;   // 赠送称号
	const ADMIN_API_GIVE_PLAYER_FEATS = 85;		//赠送功勋
	...
	//商城广告图片切换
	public static function change_ad ($ad_list) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_CHANGE_AD,
            array(
                self::pack_string(self::$ADMIN_PWD),
				self::pack_array($ad_list, array(
                    'ad_id' => 'int'
                ))
            ),
            array(
	...
	//增加玩家帮贡
    public static function increase_player_con ($player_id, $counts) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_INCREASE_CON,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id),
                self::pack_int($counts)
            ),
            array(
                'result' => 'enum',     //1-成功，0-失败
            )
	...
	//增加玩家仙令
    public static function increase_player_xian_ling ($player_id, $counts) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_INCREASE_XIAN_LING,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id),
                self::pack_int($counts)
	...
	//回档
    public static function read_backup ($player_id, $id) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_READ_BACKUP,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id),
                self::pack_int($id)
	...
	//删除物品
	 public static function delete_item ($player_item_id_list) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_DELETE_ITERM,
            array(
                self::pack_string(self::$ADMIN_PWD),
				self::pack_array($player_item_id_list, array(
                    'player_item_id' => 'int'
                ))
                //self::pack_int($player_item_id_list)
            ),
            array(
                'result' => 'enum',     //1-成功，0-失败
            )
	...
	// 赠送上供次数
    public static function give_sacrifice_count ($player_id, $normal_count, $high_class_count) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_GIVE_SACRIFICE_COUNT,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id),
                self::pack_int($normal_count),
                self::pack_int($high_class_count)
            ),
            array(
                'result' => 'enum'     //1-成功，0-失败
            )
    ...
    // 赠送玩家龙珠
    public static function give_dragonball ($player_id, $dragonball_id, $level, $num) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_GIVE_DRAGONBALL,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id),
                self::pack_int($dragonball_id),
                self::pack_int($level),
                self::pack_int($num)
            ),
            array(
                'result' => 'enum',     //1-成功，0-失败
                'success_num' => 'int'  // 成功个数
            )
        );
    }
    
    // 获取玩家战力
    public static function get_player_attack_power ($player_id) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_GET_PLAYER_ATTACK_POWER,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id)
            ),
	...
	// 获取玩家战力
    public static function team_buying_activity_award ($player_id,$id,$ingot,$message) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_TEAM_BUYING_ACTIVITY_AWARD,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id),
                self::pack_int($id),
    ...
    // 发布雇佣伙伴
    public static function publish_hire_role ($role_list, $start, $expire) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_PUBLISH_HIRE_ROLE,
            array(
                self::pack_string(self::$ADMIN_PWD),
				self::pack_array($role_list, array(
                    'role_id' => 'int'
                )),
                self::pack_int($start),
                self::pack_int($expire)
            ),
            array(
                'result' => 'enum'     //1-成功，0-失败
            )
	...
	//增加玩家八仙令
    public static function increase_ba_xian_ling ($player_id, $value) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_INCREASE_BA_XIAN_LING,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id),
                self::pack_int($value)
	...
	//增加玩家元神珠
    public static function increase_pearl ($player_id, $value) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_INCREASE_PEARL,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id),
                self::pack_int($value)
    ...
    // 发放补偿
    public static function buchang ($ver, $minlv, $maxlv, $msg, $ingot, $coin, $fame, 
        $skill, $feat, $item1, $num1, $item2, $num2, $item3, $num3)
    {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_BUCHANG,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($ver),
                self::pack_int($minlv),
	...
	//赠送称号
    public static function give_player_title ($player_id, $title_id) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_GIVE_PLAYER_TITLE,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id),
                self::pack_int($title_id)
            ),
            array(
                'result' => 'enum'     //1-成功，0-失败
            )
	...
	//赠送功勋
    public static function give_player_feats ($player_id, $value) {
        return self::invoke_api(
            self::ADMIN_API_MODULE,
            self::ADMIN_API_GIVE_PLAYER_FEATS,
            array(
                self::pack_string(self::$ADMIN_PWD),
                self::pack_int($player_id),
                self::pack_int($value)
...
//print_r(api_admin::buchang(2014080101, 1, 200, "补偿礼包", 100, 100, 100, 100, 100, 1217, 10, 0, 0, 0, 0));
//print_r(api_admin::get_player_attack_power(25));
//print_r(api_admin::delete_item(array(array('player_item_id'=>3923))));
//print_r(api_admin::read_backup(1691,5));
...
// print_r(api_admin::give_player_feats(9,50));
?>
