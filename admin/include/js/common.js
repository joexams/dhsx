...
//			div.style.width = pmwidth + 'px';
			...
			$(layer).innerHTML = '<div class="showWin" style="position: relative;top:'+topheight+'px;">' +
				'<a href="javascript:void(0)" onclick="pmwin(\'close\',\'\',\''+layer+'\');"title="关闭" style="position: absolute; right: 15px; top: 12px"><b>×</b></a>' +
				'<input type="hidden" name="showWin" /><div id="'+winmask+'"></div></div>';
//				<iframe  style="left:0;height:100%;width:770px;margin:100px auto;position:absolute; top:10px; z-index:-1;  border-style:none;filter=\'progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)\'"></iframe>';
				
		}
		$(layer).style.display = '';
		$(layer).style.top = ((clientHeight - pmheight) / 2 + scrollTop) + 'px';
		if(!param) {
			$(winmask).innerHTML = '错误参数!';
		} else {
...
function selectAjax(url,id,valArr,type){
	...
	if (valArr) {//判断是否设置传递后台提交参数
		...
		for(var i=0;i<val.length;i++){					
			...
			url	 += "&"+val[i]+"=" + unescape(nowValue);//赋值URL
		}
		url += "&time="+new Date().getTime();
	}
	if (window.location.pathname == '/t_alpha.php'){
		url += "&dbs=alpha";
	}
...
function fill_data (ele) {
  	var tag_input = ele.parentNode.parentNode.getElementsByTagName("input"); //获取同一行的所有input标签
  	var tag_length = tag_input.length;
  	text_value = ele.value;
  	values = text_value.split('\t');
  	if (values.length > 1){
	  	if (values.length != tag_length) {
	  		alert('参数无效');
	  		ele.focus();
	  		return true;
	  	}
	  	for (var i=0;i<tag_length;i++){
	  		tag_input[i].value = values[i];
	  	}
  	}
  }