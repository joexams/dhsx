...
<table class="table">
  <tr>
    <th colspan="6"><span class="bluetext"><?php echo $name?></span> 的携带物品</th>
  </tr>
...
<td><input type="checkbox" name="id_del[]" value="<?php echo $rs['id']?>" title="选择删除"/></td>
    <td><strong><?php echo $rs['item_name']?></strong> (<?php echo $rs['require_level']?>级)</td>
...
<?php if(is_array($item_list)) { foreach($item_list as $irs) { ?>
 <option value="<?php echo $irs['id']?>" ><?php echo $irs['name']?>
<?php if($irs['type_id'] <=7) { ?>
-<?php echo $irs['require_level']?>级
<?php } ?>
</option>
...
<input type="hidden" name="action" value="SetTownNpcItem" />
