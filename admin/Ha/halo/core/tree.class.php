...
class tree {
	...
	* @return string
	*/
	public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '', $str_last = ''){
		$number=1;
		...
		if(is_array($child)){
			...
			foreach($child as $id=>$value){
				...
				if ($parentid != 0){
					$level == 3 && $str_last ? eval("\$nstr = \"$str_last\";") : eval("\$nstr = \"$str\";");
				}
				$this->ret .= $nstr;
				$nbsp = $this->nbsp;
				$this->get_tree($id, $str, $sid, $adds.$k.$nbsp,$str_group,$str_last);
				$number++;
			}
		}
		return $this->ret;
	...
	*/
	...
	public function creat_sub_json($myid, $str='') {
		...
		if(is_array($sub_cats)) foreach($sub_cats as $c) {			
			$data[$n]['mid'] = iconv(CHARSET,'utf-8',$c['catid']);
