...
//		$sql = str_replace('ho_', $this->db_tablepre, $sql);
	 ...
	 * 直接执行sql查询
	 * @param $sql							查询sql语句
	 * @return	boolean/query resource		返回结果集
	 */
...
//		echo json_encode(array('status'=>0,'msg'=>$where));
		if (is_array($where)) $where = $this->sqls($where);
	...
	final public function count($where = '', $data) {
		...
		$r = $this->get_one($where, "COUNT($data) AS num");
		return $r['num'];
	}
