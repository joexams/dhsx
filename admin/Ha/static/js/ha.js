...
Ha.notify = {
	...
	show_notification_close_button: function() {
		...
		$('.ajax-notification-bar .close').animate({
			opacity: 0.2
		}, this.toggleSpeed, this.easing);
	},
	hide_notification_close_button: function() {
		$('.ajax-notification-bar .close').animate({
			opacity: 0
		}, this.toggleSpeed/3, this.easing, function() {
		...
		if(Ha.notify.visibleNotification === true) {
			...
			$('.ajax-notification-bar p').first().slideUp(Ha.notify.toggleSpeed, Ha.notify.easing, function() {
				...
				if($('.ajax-notification-bar p').length === 0) {
					...
					clearTimeout(Ha.notify.timeout);
					Ha.notify.timeout = setTimeout(Ha.notify.hideNotification, Ha.notify.delay/2);
				}
			});
		}
	}
...
Ha.page = {
	...
	getList: function(index, callback){
		...
		$.ajax({
			...
			error: function(e){
				...
				Ha.notify.show('系统出错了：'+msg, '', 'error');
			}
		});
	},
	showList: function(data){
		if (data.status > 0){
			...
			$( "#pager" ).pager({ pagenumber: Ha.page.pageIndex, pagecount: Ha.page.pageCount, word: Ha.page.word, buttonClickCallback: Ha.page.getList });
			if (data.count > 0){
...
Ha.common = {
	...
	show: function(data) {
		...
		Ha.notify.show(data.msg, 'normal', alertclassname);
	},
