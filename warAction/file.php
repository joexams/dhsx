<?php
   ...
   $changeTime = date("Y-m-d H:i");
   $isChangeTime = false;
   
   myglob($url, $url);
   function myglob ($folder, $path)
   {
       $path_pattern = $path.'/*';
	   $f = glob($path_pattern);
	   $len = count($f);
	   ...
	   global $monsters, $pngFile;

	   $datafile = str_replace(" - ", "", substr($folder, 1));
	  // $datafile = mb_convert_encoding($datafile, "utf8", "gbk"); 
	   // 是否有数据库
	   if (!array_key_exists($datafile, $monsters))
	   {
	       append_to_txtfile($datafile);
		   return;
	   }
	   
		$sign = $monsters[$datafile];
	   // $datafile = mb_convert_encoding($datafile, "gbk", "utf8"); 
	   ...
	   // 是否最新
	   if($dataList["oldUpData"] != null)
	   {
	       if($dataList["oldUpDatasTimer"] >= $pngtimer)
		   {
		      echo "isNew". $datafile. "\n";
        //              return;
		   }
	   ...
	   echo "write". $datafile. $folder . $sign."\n";
   }
	...
	*/ 
	...
	{
	    $url = substr($url, 9);
	    $list = "{url : \"file:///D:/work/". $url. "/\"" . ",  name : \"战场\", ". "sign : \"". $sign. "\"},\n{url : \"file:///D:/work/". $url. "/\"" . ",  name : \"战场-mini\", ". "sign : \"". $sign. "Mini\"},\n";
...
for(var j = 0; j < listLen; j++)
{
	...
	*/ 
	function append_to_Log($sign, $name)
	{
	   ...
	   $fp = fopen("log/".$logTime."monster". ".txt", "a");
	   $write = fputs($fp, $file. "\r\n");
	   fclose($fp);
	}
?>
