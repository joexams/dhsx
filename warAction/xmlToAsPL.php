<?php
	...
	*/
	...
	function renderXML ($fileName, $path) 
	{
	   ...
	   for($i = 0; $i < $xmlLen; $i++)
	   {
		   ...
		   $strList .= changeXMl($xmlList[$i], $height[$i]["mainHeight"], $standBy["dir"], 1);
		    $strListMini .= changeXMl($xmlList[$i], $height[$i]["mainHeight"], $standBy["dir"], 1);
	   }
	   ...
	   $signName = "";
	   $signName = $roles[$newName];
	   
   	   echo $signName.$newName. " exists!\n";
	...
	function renderAsData ($standList, $strList, $signName, $mini)
	{
	   ...
	   $as = "package com.assist.view.war.roles.players
{
	...
	*/ 
	...
	{
	   echo "write ".$file. "-------". "\n";
	   $newUrl = "../../client/com/assist/view/war/roles/players/". $file;
