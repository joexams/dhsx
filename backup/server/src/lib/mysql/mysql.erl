...
start_link(Id, Host, User, Password, Database) when is_list(Host), is_list(User), is_list(Password),
						    is_list(Database) ->
    start_link(Id, Host, ?PORT, User, Password, Database, undefined, 1, true).
...
start_link(Id, Host, User, Password, Database, LogFun) when is_list(Host), is_list(User), is_list(Password),
							    is_list(Database) ->
    start_link(Id, Host, ?PORT, User, Password, Database, LogFun, 1, true).

start_link(Id, Host, Port, User, Password, Database, LogFun, PoolSize, Reconnect) 
    when is_list(Host), is_integer(Port), is_list(User), is_list(Password), is_list(Database) ->
    crypto:start(),
    Result = gen_server:start_link({local, ?SERVER}, ?MODULE, [Id, Host, Port, User, Password, Database, LogFun, PoolSize, Reconnect], []),
    error_logger:info_msg(
        "mysql init finished~n", []
    ),
    Result.
...
%%--------------------------------------------------------------------
%% Function: get_result_insert_id(MySQLRes)
%%           MySQLRes = term(), result of fetch function on "updated"
%% Descrip.: Extract Insert Id from MySQL Result on update(INSERT)
%% Returns : InsertId
%%           InsertId = integer()
%%--------------------------------------------------------------------
...
% 初始化连接
init_connect(_Id, _Host, _Port, _User, _Password, _Database, _Reconnect, 0, ConnList, _LogFun) ->
    {ok, ConnList};
init_connect(Id, Host, Port, User, Password, Database, Reconnect, NumberCountDown, ConnList, LogFun) ->
    case mysql_conn:start(Host, Port, User, Password, Database, LogFun) of
        {ok, ConnPid} ->
            MysqlConn =
                case Reconnect of
            ...
            % 创建连接成功
            case add_mysql_conn(MysqlConn, ConnList) of
                {ok, NewConnList} ->
                    log(LogFun, normal, "mysql: Added connection with id '~p' (pid ~p) to my list",
                        [Id, ConnPid]),
                    init_connect(Id, Host, Port, User, Password, Database, Reconnect, NumberCountDown -1, NewConnList, LogFun);
                error ->
                    log(LogFun, normal, "failed adding MySQL connection to my list"),
                    {error, "Failed adding connection to list"}
                end;
        {error, Reason} ->
...
%%--------------------------------------------------------------------
init([Id, Host, Port, User, Password, Database, LogFun, MysqlPoolSize, Reconnect]) ->
    case init_connect(Id, Host, Port, User, Password, Database, Reconnect, MysqlPoolSize, [], LogFun) of
        {ok, ConnList} ->
