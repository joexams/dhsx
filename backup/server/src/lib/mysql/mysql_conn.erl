...
init(Host, Port, User, Password, Database, LogFun, Parent) ->
	...
	{ok, RecvPid, Sock} ->
		...
		{ok, Version} ->
		    case do_query(Sock, RecvPid, LogFun, " set names utf8; use " ++ Database, Version, [{result_type, binary}]) of
			{error, MySQLRes} ->
...
get_with_length_2(<<252:8, Length:16/little, Rest/binary>>) ->
    {Length, Rest};
get_with_length_2(<<253:8, Length:24/little, Rest/binary>>) ->
    {Length, Rest};
get_with_length_2(<<254:8, Length:64/little, Rest/binary>>) ->
    {Length, Rest};
get_with_length_2(<<Length:8, Rest/binary>>) when Length < 251 ->
    {Length, Rest}.
