...
$dbh->db_name = "gamedb_alpha_tw";
$client_dir = dirname(dirname(dirname(__FILE__)))."/client-tw/";
$desc_dir = $client_dir."com/assist/server/";

if (is_dir($client_dir) == false) {
	print "没有繁体版目录。";
	exit(1);
}
...
for ($i = 0; $i < count($list); $i++) {
	...
	if (false == preg_match("/0$/", $basename)) {
		if ($str != "") $str .= "\r\n";
		$str .= "			".$basename.";";
	}
}
...
print repeat("[data] template", 75, ".")."DONE.\n";
